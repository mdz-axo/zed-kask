...
|----------|------|---------|
| `code-review-scope.j2` | KnowAct | Compute the real diff, Fagan change sizing, critical-path identification, and a change model (Good Regulator). Consumes `prior_review.next_review_focus` as the primary emphasis. Does not judge findings — only models the change. |
| `code-review-perspectives.j2` | KnowAct | Multi-axis DETECTION across PERFECT-ordered axes × the addyosmani five-axis, with optional delegation to kali-audit / bug-hunt / refactor-architecture / deep-module / essentialist. Records raw, unverdicted findings with file:line + verbatim evidence (no-fiction). Detection/collection separation (Sauer) — no verdicts here. |
| `code-review-adjudicate.j2` | KnowAct | Defect COLLECTION: pragmatic-semantics IS/OUGHT + epistemic mode + provenance + constraint-force severity + a falsifier (H0/H1) + grill-me self-challenge + file:line no-fiction citation (uncited findings rejected). Computes `blocker_delta` vs `prior_review`. Does not re-scan the code. |
| `code-review-report.j2` | KnowAct | Verdict (driven by Blocker presence, not nit count) + named structural remedies + the 3 highest-leverage top fixes + coverage honesty (checked / not_checked / residual_risk — never a bare "LGTM") + `lessons_learned` / `next_review_focus` for loop closure. Review-first. |
| `code-review-implement.j2` | KnowAct | Optional Act phase: generate + apply concrete surgical fixes for findings at/above the requested tier. Runs only when `fix_mode` ∈ {blockers, should_fix, all} (gated by `step.condition`, default-deny). Reads the actual file before emitting `old_text` (never fabricates). Honors `.rules`. `applied_count` reflects actual application. |
...
- **OCAP:** delegation chain required; template-scoped; signature algorithm ed25519; capability expiry 3600s.
- Registry is authoritative — when this SKILL.md disagrees with the registry templates, the registry wins.