...
Execution order: collection setup and SCOUT → public-source discovery + shared verification at candidate commitment → remaining collection → INTEL →
...
2. For companies that pass SCOUT, before INTEL or valuation independently discover dated public issuer and regulator primary disclosures using only the resolved public issuer identifiers and an explicit as-of date. The verifier must inspect original retrieved responses and retain both search scopes, extraction responses and failures; if its actual tool grants prevent discovery, mark source review `not_checked`. Do not transmit private source-packet text as a public search query or claim that separate chat alone enforces read-only access. Freeze a small sourced factual candidate note and run the SAME `company-research/verification-handoff` source check and gate described in verify-before-publish on it now (not the publication decision). Nil/zero factual measurement or unperformed discovery stops before costly analysis. Later changes to the artifact, evidence, forecast or as-of require re-execution, not reuse of this receipt. For companies that pass the early gate, collect business context with `company_research_search`, `web_search`, `company_transcript` and the financial tools required below. Retrieve the original page or filing with `web_extract` for load-bearing claims and quotes; a search snippet or generated answer is only a lead. Retain provider warnings, entity identity, period, currency, units and audit status. A 10-Q is unaudited interim data, not an audited annual statement.
3. For issuer/regulator public discovery, retain each actual `web_search` result array, `count`, `providers_failed`, public query, scope, domain filter, limit, and failed call in `pipeline_tool_log`. Restrict the bounded search to dated relevant primary domains. Extract every returned hit into an original response and inventory it as material or reasoned `not_material`; an unresolved hit or failed provider is `not_checked`. For official PDFs unsupported by `web_extract`, retain the full downloaded binary outside the repo, record its public URL and SHA-256, and pass actual `corpus_convert` page text as `output.text` with its `method`, `origin_path` and matching digest/log. Only `method: text_extraction` can support a direct original quote; `ocr_pipeline` is model-mediated and needs separate deterministic matching against the full hashed PDF or stays `not_checked`. A logged local `pdftotext -layout` over the retained hashed original may instead supply an explicitly named `pdftotext` passage with `output.method: pdftotext`; the independent verifier must re-run it and check binary SHA-256, never relabel it as a corpus MCP tool output. The independent verifier re-hashes the full file and confirms its URL/download identity and relevant-page coverage; missing checks remain `not_checked`, never a relabelled web extraction. Retain actual responses as `source_outputs`: `{tool_name, description, output_key, output, source_kind, url, retrieved_at, period, unit}`. Use a unique `output_key` also present in `pipeline_tool_log`; unknown metadata is null. `source_kind` is `original` (retrieved passages/provider data), `derived` (computed results), or `synthesis` (generated prose). Keep source content separate from model-written summaries and record failed calls with their errors. Pass this array to `company-research/intel-mosaic` and keep it through all downstream stages.
...
5. For a large public-only packet, keep complete responses in `companies-mcp/research-runs/{run_id}/packet.json` under the user artifacts directory and hash exact bytes (SHA-256). The independent verifier calls `company_verification_packet_check(run_id, expected_sha256)` to run the SAME source-check form without an inline payload; it independently checks original file identities and materiality before treating a mechanical `checked` as complete. Do not store private unpublished text in that plaintext route; without authorized private storage report `not_checked`. Never replace full originals with the research ledger's short excerpts. Before release, read `get_research_run`, disclose recording limitations and annotate only checks actually performed. A source-level ledger annotation is not claim-level verification.
...
2. Render the SAME `company-research/verification-handoff` used at candidate commitment with current `target_text`, resolved public `issuer_identifier`, explicit `as_of_date`, accumulated `source_outputs`, `pipeline_tool_log`, independently refreshed `disclosure_inventory` (material original-source keys, exact quotes and report markers), frozen `original_forecast` and current `working_forecast` (both null when no forecast), applicable `leak_rules`, `congruence_rules`, and `historical_findings` (empty first pass). Call `spawn_agent` with label `verify-flash-report` and the rendered handoff or a NEW canonical packet run ID and SHA-256 for this exact target; the independent verifier calls `company_verification_packet_check` for large packets, then performs the remaining factual and materiality checks. Do not inherit a prior packet digest or a mechanical-only tool verdict as publication approval. The verifier must load `grounding-verify`, run original-source mechanical checks, and independently reconcile every bounded public issuer/regulator search result to retrieved original content using only public identifiers; it must not substitute a generated answer or training knowledge. An unavailable discovery capability is `not_checked`, never a clean inventory. Delegation/file-access failure is `incomplete`, not permission to self-approve.
3. Consume the canonical `fact_score`, `fact_score_breakdown`, `verified_claims`, `hallucination_findings`, `source_conflicts`, `source_review`, `forecast_integrity`, `data_gaps`, `confidence_adjustment`, `confidence_band`, `decoupling`, and `verification_scope_limitations`. Check full target coverage and executed mechanical results; run the handoff's source-check `lisp_eval` on originals and the observed log with `max_steps=1000000` and `max_depth=4096` for a bounded full-original packet; exhausted budget is `not_checked`, not permission to substitute excerpts. Bind the first of its `[coverage_status, known_omission]` results to `source_review_status` (`not_checked` if independent discovery/materiality adjudication did not occur); preserve the second as a separate source-review finding and bind `material_failure=true` when it is `material_omission`, even if discovery remains incomplete. Bind raw original and working forecast snapshots; then execute the shared gate. Never accept a report author's `checked` field as an oracle. `claims_checked` comes from `fact_score_breakdown.claims_checked`; missing fields are not zero/default success.
4. Gate `incomplete` (including nil, zero claims, omitted sections, unperformed primary discovery, forecast snapshot absence or in-thread verification): stop publication and return a labelled incomplete draft with the blocker. Gate `needs_work` (score < 0.60 or material failure): re-enter the earliest affected collection/synthesis stage, correct the claim or source and regenerate dependent outputs, including VALUATION/COMMUNICATION/LENS when affected, then re-verify. Preserve earlier records unchanged, keyed by iteration and stage. Do not remove material uncertainty just to improve a score. All retries share the existing maximum of three pipeline iterations; no new retry budget per gate.
...
- Step 6 reuses the full `listening` skill as a sub-process (MAIA v3 earnings-call listening, no-fabrication invariant).
...
- The THESIS quality gate in the deep pipeline uses `company-research/thesis-judge` (semantic evaluation), not self-assessment — to avoid the LLM-improves-against-LLM-scored-target trap.
