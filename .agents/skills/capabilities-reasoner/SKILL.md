...
- **Attenuation** = the proposed intervention for ceiling violations. A capability
  token's scope narrows on delegation [^aip-2026]; the reasoner can describe
  a proposed narrowing, but cannot revoke, sign, or issue a token.
...
- **Persistence-grounded learning** (floor): read prior outputs from MCP
  persistence before the process starts.
...
| `grep` + manual analysis | Prerequisite DAG analysis | Evaluate phase — check CMMI maturity gates by traversing the prerequisite graph |
...
- The attenuation rule is inviolable: authority may only narrow without re-authorization. Widening requires explicit re-authorization with a recorded warrant. A proposed token or revocation is not effective authority; this skill cannot sign, issue, or revoke tokens.
