...
9. When `skill_catalog` is provided (v0.31.0): include a `skill_match_query` field per task — a natural-language capability description consumed by skill-discovery (route). Do NOT match skills yourself; just describe the capability need. Omit the field when `skill_catalog` is absent.
