...
| Template | Purpose |
|----------|---------|
| `expectation-inquiry.j2` | Gate a sourced expectation mismatch against the active user goal before selecting a discriminating next probe; on re-render reconcile actual evidence or stop on operator-declared irrelevance. No gradient shape is inferred from one observation. |
