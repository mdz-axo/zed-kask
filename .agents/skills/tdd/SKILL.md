...
- Verifying TDD cycle completion — checking all tests pass, clippy is clean, no `todo!()`/`unimplemented!()` stubs remain, contract structure is complete, and codegraph entity anchoring is intact.
- Performing codegraph gap analysis — comparing codegraph entities against tested behaviors, scoring expectation quality (0–3), cross-referencing goal-principle alignment against MDS categories derived from graph position, and producing deferral recommendations for `OPEN_QUESTIONS.md`.
...
- Exploring for codegraph blind spots — dispatching to the `bug-hunt` skill with a charter scoped to the slice's code, finding bugs the codegraph structure never surfaced and routing them back as new entities needing tests.
...
2. Query the codegraph (`codegraph_query` MCP tool) for the entities (functions, structs, traits) in scope. Classify each using Dublin Core: `dcterms:type` = SymbolKind, `dcterms:identifier` = qualified name, `dcterms:source` = file:line, `dcterms:subject` = MDS category DERIVED from the entity's graph position (what crate/module it's in), not looked up from a spec. Traverse callers (`codegraph_traverse` direction=incoming) to understand what depends on each entity.
...
13. Emit a `risk_profile` output: `highest_priority` (P0 > P1 > P2 — the most severe priority in the slice), `touches_trust` (true when any behavior is P0 / Trust), `trust_behaviors` (the P0 behavior descriptions). This drives step 7 (explore) to dispatch to `bug-hunt` on Trust code even when codegraph coverage looks complete — Trust code has the highest cost of failure and warrants exploratory testing regardless of codegraph coverage.
...
: Verify each test carries a contract annotation (`expect:` + `[P{N}]`) anchored to a valid codegraph entity (`dcterms:identifier`).
5. Confirm no codegraph entity in scope is missing a tracer bullet.
...
: Match each tested behavior to a codegraph entity via its contract annotation (`expect:` + `[P{N}]` anchored to `dcterms:identifier`); flag behaviors without annotations as unanchored.
2. Identify gaps: codegraph entities with no matching tested behavior.
3. Derive priority from MDS category: P0 = Trust, P1 = Domain/Composition, P2 = Lifecycle/Curation.
: For codegraph entities with no callers (orphaned in the graph), flag as additional gaps — orphaned code is untested code.
...
: Cross-reference MDS category alignment: confirm the contract's goal principle matches the MDS category derived from the codegraph entity's position; flag deviations with rationale.
11. Check Magna Carta completeness: list which of P1–P4 are missing from constraining annotations per covered requirement.
: Ensure every codegraph entity appears in exactly one of: `covered_entities`, `gaps`, or `deferrals`.
...
3. Dispatch to the `bug-hunt` skill with a charter scoped to the slice's code (`charter_focus`: codegraph blind spots — behaviors the code structure did not surface).
4. Collect bug-hunt's findings (each cites file:line with verbatim evidence — bug-hunt's no-fiction rule).
5. Classify each finding: a Tier-1 BUG not covered by an existing tracer bullet is a new gap (codegraph blind spot) → route to `replan`; a BUG covered by an existing test means the test is weak → route to `replan` (strengthen via proptest); P2 findings may defer.
6. Route: any P0/P1 new gaps → `replan` (findings become new codegraph entities → new tracer bullets); P2-only or no new gaps → `converge`.
...
|----------|------|---------|
| `tdd-plan.j2` | KnowAct | Plan a TDD cycle: query the codegraph for entities with Dublin Core classification with goal-principle anchoring (each requirement names its P{N} principle), identify public interfaces, classify behaviors by MDS category with default goal-principle mapping, prioritize by risk (P0-P2+), and get user approval before writing any code. |
| `tdd-tracer.j2` | KnowAct | Execute a tracer bullet: write ONE failing test for ONE behavior anchored to a codegraph entity AND goal principle via expect: field with [P{N}] tag (per PRINCIPLES.md §1.6). Contract-first: full v0.28.0 structure with expect:, [P{N}] Constraining: annotations, and pre:/post:. Then minimal code to satisfy the contract. |
| `tdd-strengthen.j2` | KnowAct | Strengthen a GREEN tracer bullet by dispatching to the proptest skill for the universal test that verifies the contract's post:/inv: across the full input space. Uses programmatic oracles (oracle_reference / oracle_invariant) per the HarnessLLM constraint. A proptest fail is a second source of RED routing back to the tracer (impl wrong) or the plan (contract wrong). Skips cleanly for hardcoded (fixed-value) contracts. Execution mode: standalone (TDD can run tests). |
...
graph TD
    ...
    BH["bug-hunt — codegraph blind spots\n(exploratory)"]
...
- **bug-hunt**: exploratory bug finder. TDD dispatches to bug-hunt at step 7
  ...
  finds codegraph blind spots (bugs the code structure never surfaced); findings not covered by an
  existing tracer bullet become new codegraph entities routing back to the
