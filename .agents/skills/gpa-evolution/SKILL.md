...
6. **Check convergence.** No tool computes hypervolume, so a `hypervolume_delta` supplied by the model would be an unmeasured number. Until one exists, call `lisp_eval` with form `(if (< iteration 2) 1.0 (max 0 (min 1 (* 0.05 new_members))))` over the count of new non-dominated members from the measured frontier update, and report hypervolume as `unverified`. Converged when the metric ≤ threshold (default 0.10). Bound: at most 5 iterations per session; on reaching it, stop and report the frontier and the remaining metric.
