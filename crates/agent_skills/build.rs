...
fn main() {
  ...
  \
    ...
    if templates_dir.is_dir() {
        collect_template_files(&templates_dir, &templates_dir, &mut template_entries);
    }
    template_entries.sort_by(|a, b| a.0.cmp(&b.0));

    let mut template_code = String::new();
    template_code
        .push_str("// Auto-generated by build.rs — seed payload for kask registry templates.\n");
    template_code.push_str("// Do not edit manually. Regenerated on every build.\n\n");
    template_code.push_str(
        "pub(crate) static SHIPPED_TEMPLATE_SEED_ENTRIES: &[(&str, &str)] = &[
",
    );
    for (rel, path) in &template_entries {
        let abs = path.to_str().expect("UTF-8 template path");
...
",
    ...
    // ── Classifier-config seed payload ──
    // kask/registry/classify/*.yaml are runtime-consumed non-template assets:
    // the corpus server loads them via `load_classifier_config` from
    // `{registry}/classify/{name}.yaml`. Embed them so `seed_templates`
    // writes them next to the seeded templates (`{data}/skills/registry/
    // classify/`) — without this, non-default classifiers fail in production
    // where no source checkout exists.
    let classify_dir = repo_root.join("kask/registry/classify");
    let mut classify_entries = Vec::new();
    if classify_dir.is_dir() {
        collect_extension_files(&classify_dir, &classify_dir, "yaml", &mut classify_entries);
    }
    classify_entries.sort_by(|a, b| a.0.cmp(&b.0));

    let mut classify_code = String::new();
    classify_code
        .push_str("// Auto-generated by build.rs — seed payload for kask classifier configs.\n");
    classify_code.push_str("// Do not edit manually. Regenerated on every build.\n\n");
    classify_code.push_str(
        "pub(crate) static SHIPPED_CLASSIFY_SEED_ENTRIES: &[(&str, &str)] = &[
",
    );
    for (rel, path) in &classify_entries {
        let abs = path.to_str().expect("UTF-8 classify config path");
        classify_code.push_str(&format!(
            "    (\"{rel}\", include_str!(\"{abs}\")),
"
        ));
    }
...
",
    ...
    fs::write(out_dir.join("embedded_classify.rs"), classify_code)
        .expect("write generated embedded_classify.rs");

    // Rebuild when templates or classifier configs change.
    if templates_dir.is_dir() {
        println!("cargo:rerun-if-changed={}", templates_dir.display());
        for (_, path) in &template_entries {
            println!("cargo:rerun-if-changed={}", path.display());
        }
    }
    if classify_dir.is_dir() {
        println!("cargo:rerun-if-changed={}", classify_dir.display());
        for (_, path) in &classify_entries {
            println!("cargo:rerun-if-changed={}", path.display());
        }
    }
    ...
    for entry in fs::read_dir(dir).expect("read template directory") {
        ...
        if path.is_dir() {
            ...
            let rel = path
                ...
                .to_str()
                .expect("UTF-8 template relative path")
                .to_string();
            out.push((rel, path));
        }
    }
