    ...
    fn select_model(&self, model_id: AgentModelId, cx: &mut App) -> Task<Result<()>> {
        ...
        thread.update(cx, |thread, cx| {
            ...
            thread.set_reasoning_effort(
                enable_thinking.then(|| effort.clone().unwrap_or_else(|| "default".to_string())),
                cx,
            );
...
mod internal_tests {
    ...
    async fn test_explicit_subagent_model_preserves_configured_settings(cx: &mut TestAppContext) {
        ...
        subagent_thread.read_with(cx, |thread, _| {
            ...
            assert!(thread.reasoning_effort().is_some());
            assert_eq!(thread.reasoning_effort(), Some(&"high".to_string()));
