...
fn address() -> SocketAddr {
    ...
    let port = match *release_channel::RELEASE_CHANNEL {
        // zed-kask uses a distinct port block (offset +500) so it coexists
        // with an upstream zed install on the same machine.
        ReleaseChannel::Dev => 43737 + 500,
        ReleaseChannel::Preview => 43737 + 500 + USER_BLOCK,
        ReleaseChannel::Stable => 43737 + 500 + (2 * USER_BLOCK),
        ReleaseChannel::Nightly => 43737 + 500 + (3 * USER_BLOCK),
    };
...
fn instance_handshake() -> &'static str {
    match *release_channel::RELEASE_CHANNEL {
        ReleaseChannel::Dev => "Zed-Kask Dev Instance Running",
        ReleaseChannel::Nightly => "Zed-Kask Nightly Instance Running",
        ReleaseChannel::Preview => "Zed-Kask Preview Instance Running",
        ReleaseChannel::Stable => "Zed-Kask Stable Instance Running",
    }
}
