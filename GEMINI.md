...
* When a prompt references an external source (URL, financial statement, codebase), delegate extraction to a specialized skill (structured-extraction, metacognition's inquiry experiment — do not ask the user to transcribe data that already exists in a machine-readable source. Transcription requests shift cognitive load to the user and introduce transcription errors.
...
## API keys live at exactly one keychain location

Data-service API keys (Exa, Tavily, Brave, SerpAPI, Firecrawl, FMP, EODHD, FRED, Nebius, HF Token, ABW, etc.) are stored in zed's `CredentialsProvider` keychain namespace under `kask://credentials/<key>` (label `zed-github-account`, attribute `url=kask://credentials/<key>`). Inference-provider API keys (OpenRouter, DeepInfra, RunPod, KiloCode) live at exactly one other location: the provider's `api_url` keychain slot (`https://openrouter.ai/api/v1`, `https://api.deepinfra.com/v1/openai`, `https://api.runpod.io`, `https://api.kilo.ai/api/gateway`) — the same slot zed's `ApiKeyState` reads and Settings → AI → LLM Providers writes; nothing reads or writes `kask://credentials/{openrouter,deepinfra,runpod,kilocode}`. There is NO `service=hkask` keychain fallback for API keys — `resolve_credential` reads API keys from env vars only (injected by `build_mcp_server_env`, which reads both slot kinds from zed's keychain). The legacy `service=hkask` namespace is fully removed and purged at startup (`hkask-keystore/src/keychain.rs`) — do not write anything to it; entries silently vanish. The single internal key `hkask_db_passphrase` lives in the `kask://credentials/` namespace; there is one passphrase for all SQLCipher DBs and no separate swarm-memory passphrase. Do NOT add `*_enabled` settings toggles — the key's presence in the keychain IS the toggle.

## Canonical `HKASK_DB_PASSPHRASE` resolution helper

All MCP servers that consume `HKASK_DB_PASSPHRASE` must use `hkask_mcp_server::server::resolve_db_passphrase(&ctx.credentials)` — a 2-tier chain (ctx.credentials → `resolve_credential` which does env → `hkask-keystore` keychain `hkask-db-passphrase`) — not inline re-implementations. `ServerContext::resolve_db_credential` delegates to it. Reference pattern: `hkask-mcp-kata-kanban`. Corpus captures the resolved passphrase at server construction into `static CORPUS_DB_PASSPHRASE: OnceLock<Option<String>>`. First-run provisioning: `hkask_keystore::keychain::provision_db_passphrase_string` owns the whole chain (env → keychain → first-run default `"allostery"`, empty/whitespace env treated as unset); the bridge's `identity.rs` adapters route through it — no inline passphrase logic in the bridge. Called at governed MCP server launch — no mirror step.

## `nudge_mcp_servers` restart trigger after keychain writes

`write_credential` / `delete_credential` in `crates/settings_ui/src/pages/kask_page.rs` call `nudge_mcp_servers(cx)` after a keychain write/delete for any credential URL that feeds MCP server env. The predicate is `credential_url_feeds_mcp_servers` — membership in `credential_urls_for_mcp()`'s set, which covers `kask://credentials/<key>` data-service slots and the inference providers' `api_url` slots alike. The nudge fires `SettingsStore::notify_observers` → `sync_kask_mcp_runtime_servers` → `build_mcp_server_env` (re-reads keychain) → restart. Without this, a keychain write doesn't trigger a server restart and the server keeps reading the old key until next launch.
