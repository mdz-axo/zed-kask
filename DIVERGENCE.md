...
| D10 | Kask panel | `crates/kask_panel/` | Native GPUI center-pane `Item` (not a dock `Panel`). Tab strip (10 built-in MCP servers). Per-tab `CuratorSession` (stateful, streaming via `InferencePort::generate_stream_with_messages`). Direct `/tool args` invocation (OCAP-gated, renders as tool-call cards). Rich markdown + mermaid rendering. Jinja2 system-prompt templates. KVP persistence. `kask_panel::Toggle` / `ToggleFocus` actions. Deployed on demand via `kask_panel::init(cx)`. Per-server visualization views (portfolio, scenarios, kanban). |
