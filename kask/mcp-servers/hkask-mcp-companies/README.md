...
- Financial-data tools route eligible symbol lookups between FMP and EODHD. `company_screener` has one execution surface with two compositions: immediate ad hoc filtering, and FactSet-shaped saved screens (`calculate` → `status` → paginated `results`). The generated tool schema exposes the registered templates' context fields so schema-guided callers can populate them. Saved Jinja templates validate supplied context against `[inference] contract.input`, receive the current UTC `as_of` through `contract.server_input`, and report all missing fields together before Jinja2-compatible rendering; templates and direct API definitions then deserialize to the same typed screen document. Current-only acquisition rejects historical direct-definition labels and empty exchange universes. Calculate validates plan assertions with sandboxed `hkask-lisp`, persists and returns a queued job, then freezes the EODHD universe and executes derived columns in the background. It stores one immutable columnar result with per-column provenance and never recalculates during pagination. Returned calculation errors, unwind panics, missing job transitions, and startup-interrupted `queued` or `executing` jobs surface as failures because process-local universe snapshots cannot be resumed. The `expectations_gap` template applies USD-cap and `adjusted_close × avgvol_200d` USD-liquidity filters before fundamentals, groups qualifying securities through connected LEI, primary-ticker, ISIN, and non-conflicting normalized-name evidence, and computes price-implied growth/profitability columns as a set. Results expose the canonical issuer key and grouping provenance, every eligible traded line, the analysis line, the highest-liquidity actionable line, and price/currency-normalization provenance. `research_search` remains a later company-research capability, not a screening stage.
- The DCF projection is a two-stage model using a Gordon-growth terminal value. It models revenue, COGS, gross profit, SG&A, D&A, EBIT, tax, NOPAT, capex, net working-capital change, and free cash flow. It does not model an exit-multiple terminal method or other non-operating assets in the equity bridge.
...
`src/acquisition_tests.rs` covers acquisition order, date joins and supplement failures, EODHD normalization, legacy/warm-cache behavior and provenance, target/peer cache and learning policy, concurrent peers, DCF equivalence/guards, listing-price to statement-currency normalization, and the screener's per-exchange fan-out, unconverted USD bounds, and offset-cap pagination (parser contracts are pinned in `src/screener.rs` tests). The library's live checks skip without provider keys. `tests/fmp_endpoint_schema.rs` calls real FMP endpoints when `HKASK_FMP_API_KEY` is set; it is not an offline fixture suite. These tests call real tool handlers but do not exercise MCP transport framing.
