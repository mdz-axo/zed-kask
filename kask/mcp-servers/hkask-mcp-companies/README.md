...
- Financial-data tools route eligible symbol lookups between FMP and EODHD. `company_screener` has one execution surface with two compositions: immediate ad hoc filtering, and FactSet-shaped saved screens (`calculate` → `status` → paginated `results`). The generated tool schema exposes the registered templates' context fields so schema-guided callers can populate them. Saved Jinja templates validate supplied context against their declared `[inference] contract.input` before Jinja2-compatible rendering and report all missing fields together; templates and direct API definitions then deserialize to the same typed screen document. Current-only acquisition rejects historical `as_of` labels and empty exchange universes. Calculate currently freezes one EODHD universe snapshot before returning, then validates plan assertions with sandboxed `hkask-lisp`, executes derived columns in the background, stores an immutable columnar result with per-column provenance, and never recalculates during pagination. Returned calculation errors, unwind panics, missing job transitions, and startup-interrupted `queued` or `executing` jobs surface as failures because process-local universe snapshots cannot be resumed. The `expectations_gap` template applies USD-cap and `adjusted_close × avgvol_200d` USD-liquidity filters before fundamentals, groups qualifying securities by issuer/primary ticker, and computes price-implied growth/profitability columns as a set. `research_search` remains a later company-research capability, not a screening stage.
