...
| `company_research_search` | Search Exa, Tavily, and Brave for company-specific fundamental-research claims. |
...
| `HKASK_INVESTOR_REQUIRED_RETURN` | Yes (managed setting) | Investor equity hurdle emitted from `kask.companies.investor_required_return`; the companies server has no hardcoded fallback. MAIA default is configured once in `KaskCompaniesSettings::default`. |
...
- Discounting follows the MAIA investor perspective (`MA_Guidebook_July23.md`, “A Note on Valuation” and “Valuation”): the equity component is `kask.companies.investor_required_return`, with MAIA's 15% value defined only as that setting's default; the standalone tool may explicitly override it per request. Modified WACC = equity weight × investor target return + debt weight × pre-tax debt cost × (1 − tax rate). For financial issuers the target return is used directly.
