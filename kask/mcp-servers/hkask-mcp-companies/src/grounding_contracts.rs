...
mod tests {
    ...
    /// Every contract must have a non-empty `field_sources` map and a
    /// non-empty `agent_type`. A contract with empty fields is a no-op —
    /// it would register but ground nothing.
    #[test]
    fn contracts_are_non_empty() {
        let contracts = [
            raw_provider_data_contract(),
            valuation_contract(),
            analysis_contract(),
            portfolio_contract(),
            forecast_contract(),
        ...
        for contract in &contracts {
            assert!(
                !contract.agent_type.is_empty(),
                "agent_type must be non-empty"
            );
            assert!(
                !contract.field_sources.is_empty(),
                "field_sources must be non-empty for {}",
                contract.agent_type
            );
        }
    }

    /// Every FieldSpec must have a `why` of at least 40 chars (the card
    /// contract validator enforces this — the built-in contracts should
    /// meet the same bar).
    #[test]
    fn field_specs_have_adequate_why() {
        let contracts = [
            raw_provider_data_contract(),
            valuation_contract(),
            analysis_contract(),
        ...
        for contract in &contracts {
            for (field, spec) in &contract.field_sources {
                assert!(
                    spec.why.len() >= 40,
                    "field '{}' in contract '{}' has a 'why' of {} chars, need ≥40: {}",
                    field,
                    contract.agent_type,
                    spec.why.len(),
                    spec.why
    ...
    /// `register_all` must not panic. It registers 6 contracts with the
    /// store. The store's `has_contract` must return true for each
    /// registered agent_type.
    #[test]
    fn register_all_registers_all_contracts() {
        let store = hkask_verification::VerificationStore::in_memory();
        register_all(&store);
        // The 6 contract agent_types.
        for agent_type in [
            "company_profile",
            "dcf_valuation",
            "moat_check",
            ...
            assert!(
                store.has_contract(agent_type),
                "contract for '{}' must be registered after register_all",
                agent_type
            );
        }
    }
