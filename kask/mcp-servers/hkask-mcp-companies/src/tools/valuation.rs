...
impl CompaniesServer {
    ...
    pub async fn sensitivity_analysis(
        ...
        execute_tool(self, "sensitivity_analysis", async {
            ...
            let base_model = financial_model::project_financial_model(&hist, &assumptions)
    ...
    pub async fn monte_carlo_dcf(
        ...
        execute_tool(self, "monte_carlo_dcf", async {
            ...
            let result = financial_model::monte_carlo_dcf(
                &hist, &assumptions, sims, &ranges, current_price, &mut rng,
            )
            .map_err(|error| McpToolError::invalid_argument(error.to_string()))?;
