//! `company_transcript` tool — earnings mode.
//!
//! Thin MCP wrapper over `crate::transcript`. The deterministic, provider-shaped
...
impl CompaniesServer {
    #[tool(
        description = "Fetch company earnings-call transcripts (FMP). Coverage-honest: per-quarter gaps reported in coverage.missing, never filled. Use (year, quarter) as the temporal key — the date field is unreliable."
    )]
    pub async fn company_transcript(
        ...
        execute_tool(self, "company_transcript", async {
            ...
            // The tool fails only when zero quarters succeed — a partial result
            // with gaps is a valid, honest output (design §(a)).
            if result.transcripts.is_empty() && !result.coverage.missing.is_empty() {
                let all_no_call = result
                    .coverage
                    .missing
                    .iter()
                    .all(|missing| matches!(missing.reason, transcript::MissingReason::NoCall));
                if all_no_call {
            ...
            serde_json::to_value(&result).map_err(|error| {
                McpToolError::internal(format!("failed to serialize transcript result: {error}"))
            })
        })
        .await
    }
...
/// Resolve the window end `(year, quarter)`. Both provided → use them; one
/// provided → error; neither → infer the most recent completed quarter from
/// the current UTC date (inference from a real signal, not fabrication).
