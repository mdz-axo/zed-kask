...
mod tests {
    ...
    // P1 regression: `find_similar` must surface a missing EXA credential as
    // `WebError::NoProviderConfigured` (which maps to `permission_denied`),
    // NOT `WebError::NoProvider` (which maps to `unavailable`). The
    // distinction matters: `unavailable` tells the operator "the provider is
    // down, retry later"; `permission_denied` tells the operator "you never
    // configured this provider, set HKASK_EXA_API_KEY." A future refactor
    // that collapses `NoProviderConfigured` back into `NoProvider` would
    // re-introduce the broken feedback loop where a credential gap looks
    // like a transient outage.
    //
    // We construct a `ProviderPool` with `exa: None` and no providers — the
    // minimal configuration that exercises the guard. The guard runs before
    // any HTTP call, so this is a pure unit test with no network.
    #[tokio::test]
    async fn find_similar_returns_no_provider_configured_when_exa_missing() {
        let pool = ProviderPool::new(vec![], vec![], vec![], None);
        ...
        // from a transient outage. Compute before destructuring `msg`.
        assert_eq!(
            err.kind(),
            hkask_types::McpErrorKind::PermissionDenied,
            "NoProviderConfigured must classify as PermissionDenied, not Unavailable",
        );
        match err {
            WebError::NoProviderConfigured(msg) => {
                assert!(
                    msg.contains("HKASK_EXA_API_KEY"),
                    "NoProviderConfigured message must name HKASK_EXA_API_KEY so the \
                     operator knows which credential to set; got: {msg}",
                );
            }
            other => panic!(
                "find_similar with no Exa must return NoProviderConfigured, not {:?}",
                other
            ),
        }
    }
