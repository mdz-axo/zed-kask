...
/// Compute the blake3 hex digest of extracted content (content-hash pinning).
pub fn citation_content_hash(content: &str) -> String {
    blake3::hash(content.as_bytes()).to_hex().to_string()
}

/// Deterministic citation ID: `cite:{blake3(url + content_hash)[:16]}`.
/// Re-extracting the same content from the same URL yields the same ID.
pub fn citation_id_for(url: &str, content_hash: &str) -> String {
    let digest = blake3::hash(format!("{url}{content_hash}").as_bytes()).to_hex();
    format!("cite:{}", &digest[..16])
}

/// Upsert a citation row and return its citation_id.
///
/// The ID is a pure function of (url, content_hash), so an upsert with the
/// same inputs overwrites the same row — idempotent by construction.
pub fn upsert_citation(
    ...
    let content_hash = citation_content_hash(content);
    let citation_id = citation_id_for(url, &content_hash);
    conn.execute(
        "INSERT INTO citations (citation_id, url, content_hash, title, provider, content, span_map)
...
pub fn get_citation(
    ...
    let row = conn.query_row(
        "SELECT citation_id, url, content_hash, title, provider, fetched_at, content, span_map
         FROM citations WHERE citation_id = ?1",
        [citation_id],
        |row| {
            let span_map: Option<String> = row.get("span_map")?;
            // span_map is stored as raw JSON text; reparse so the response
            // embeds it as a JSON value, not an escaped string.
            let span_map_value = span_map
                .as_deref()
                .and_then(|s| serde_json::from_str::<serde_json::Value>(s).ok());
            Ok(serde_json::json!({
                "citation_id": row.get::<_, String>("citation_id")?,
                "url": row.get::<_, String>("url")?,
                "content_hash": row.get::<_, String>("content_hash")?,
                "title": row.get::<_, Option<String>>("title")?,
                "provider": row.get::<_, Option<String>>("provider")?,
                "fetched_at": row.get::<_, Option<String>>("fetched_at")?,
                "content": row.get::<_, String>("content")?,
                "span_map": span_map_value,
            }))
        },
    );
    match row {
...
pub fn list_citations(
    ...
    // Listings exclude the (potentially large) content body; citation_get
    ...
    const COLS: &str = "citation_id, url, content_hash, title, provider, fetched_at";
    if let Some(filter) = url_filter {
        ...
        query_and_collect(
            conn,
            &format!("SELECT {COLS} FROM citations ORDER BY fetched_at DESC LIMIT ?1"),
...
mod tests {
    ...
    fn test_conn() -> Connection {
        ...
        conn.execute_batch(RSS_SCHEMA_DDL).expect("schema applies");
        conn
    }

    #[test]
    fn citation_id_is_deterministic() {
        let hash = citation_content_hash("some extracted content");
        let first = citation_id_for("https://example.com/a", &hash);
        let second = citation_id_for("https://example.com/a", &hash);
        assert_eq!(first, second);
        assert!(first.starts_with("cite:"));
        assert_eq!(first.len(), "cite:".len() + 16);
    }

    #[test]
    fn citation_id_changes_with_content() {
        let url = "https://example.com/a";
        let id_one = citation_id_for(url, &citation_content_hash("content one"));
        let id_two = citation_id_for(url, &citation_content_hash("content two"));
        assert_ne!(id_one, id_two);
    }

    #[test]
    fn citation_store_get_round_trip() {
        let conn = test_conn();
        let (id, hash) = upsert_citation(
            &conn,
            "https://example.com/page",
            Some("A Page"),
            Some("rawfetch"),
            "full extracted body",
            Some(r#"{"claims":[{"start":0,"end":4}]}"#),
        )
        .expect("upsert succeeds");

        let row = get_citation(&conn, &id)
            .expect("get succeeds")
            .expect("row exists");
        assert_eq!(row["citation_id"].as_str(), Some(id.as_str()));
        assert_eq!(row["content_hash"].as_str(), Some(hash.as_str()));
        assert_eq!(row["url"].as_str(), Some("https://example.com/page"));
        assert_eq!(row["title"].as_str(), Some("A Page"));
        assert_eq!(row["provider"].as_str(), Some("rawfetch"));
        assert_eq!(row["content"].as_str(), Some("full extracted body"));
        assert_eq!(row["span_map"]["claims"][0]["end"].as_i64(), Some(4));
    }

    #[test]
    fn citation_get_missing_returns_none() {
        let conn = test_conn();
        let row = get_citation(&conn, "cite:0000000000000000").expect("get succeeds");
        assert!(row.is_none());
    }

    #[test]
    fn citation_upsert_is_idempotent() {
        let conn = test_conn();
        ...
        assert_eq!(first, second, "same url+content must yield the same ID");
        ...
        let row = get_citation(&conn, &first.0)
            .expect("get succeeds")
            .expect("row exists");
        assert_eq!(row["title"].as_str(), Some("Title B"));
    }

    #[test]
    fn citation_list_orders_recent_first_and_filters_by_url() {
        let conn = test_conn();
        upsert_citation(&conn, "https://example.com/a", None, None, "body a", None)
            .expect("upsert a");
        upsert_citation(&conn, "https://example.com/b", None, None, "body b", None)
            .expect("upsert b");

        let all = list_citations(&conn, None, 10).expect("list all");
        assert_eq!(all.len(), 2);

        let filtered =
            list_citations(&conn, Some("https://example.com/a"), 10).expect("list filtered");
        assert_eq!(filtered.len(), 1);
        assert_eq!(filtered[0]["url"].as_str(), Some("https://example.com/a"));

        let limited = list_citations(&conn, None, 1).expect("list limited");
