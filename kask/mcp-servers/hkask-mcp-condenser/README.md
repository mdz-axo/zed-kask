...
Without `HKASK_DB_PATH`, `condenser_persist` returns a permission-denied error and `condenser_score_saliency` with `against="memory"` returns 0.5 (neutral). All other tools work without configuration (graceful degradation). Thread summarization uses the centralized hKask inference router (configured via standard `DEEPINFRA_API_KEY`, `FALAI_API_KEY`, `OPENROUTER_API_KEY`, `KILOCODE_API_KEY` environment variables). The default model is configured via the `INFERENCE_MODEL` env var (defaults to `google/gemma-4-26B-A4B-it`).
