...
mod schema_tests {
    ...
    /// Assert a request struct's schemars schema has no bare-boolean
    /// schema-valued positions. schemars renders `serde_json::Value` as the
    /// bare boolean `true` in schema-valued positions, which strict-schema
    /// providers (Ollama, Gemini) reject (`400 cannot unmarshal bool into
    /// api.ToolProperty`). One bare boolean in any enabled tool schema fails
    /// the whole chat-completion request. The `.rules` "AnyJsonValue" trap.
    macro_rules! schema_clean_test {
        ($test_name:ident, $ty:ty) => {
            ...
            fn $test_name() {
                ...
                assert!(
                    violations.is_empty(),
                    "{} schema has bare-boolean schema positions \
    ...
    schema_clean_test!(ai_assist_request_schema, AiAssistRequest);
    ...
    schema_clean_test!(evaluate_local_request_schema, EvaluateLocalRequest);
    schema_clean_test!(execute_agent_request_schema, ExecuteAgentRequest);
    schema_clean_test!(execute_plan_local_request_schema, ExecutePlanLocalRequest);
