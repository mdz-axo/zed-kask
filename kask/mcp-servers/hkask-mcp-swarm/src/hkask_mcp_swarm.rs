...
//!
//! ## Tools (52 — both tool sets always available in either mode)
...
// `src/*.rs`. The single source of truth — the `tool_surface_is_exactly_53_registered_tools`
// test below asserts this matches the live `combined_router()` surface, so
// drift is caught at test time. Consumers across the seam (e.g. `swarm_panel`)
// re-export this const as `parse::SWARM_TOOLS`.
...
pub use crate::local_runtime::{
    LazyLocalSwarmRuntime, LocalDelegateResult, LocalSwarmRuntime, SwarmDelegationCounter,
    TaskSuccessProvenance, TaskSuccessVerdict, open_delegation_counter,
};
