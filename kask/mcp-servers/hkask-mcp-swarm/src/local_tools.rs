...
impl SwarmServer {
    ...
    /// inference port — composition guidance is generated by the LLM.
    ...
    /// as the `task`; the inference port handles JSON-object tasks.
    /// context as top-level template variables.
    #[tool(
        description = "AI assist for the swarm panel authoring forms (agent/swarm). Suggests completions for partial inputs or validates well-formedness. Authoring aid — read-only, spends nothing. Uses the inference port directly to generate composition guidance. The agent can call the skill tool with swarm-compose-guide for full guidance. The mode field (abw/local) tailors the guidance; no ABW calls in either mode."
    )]
    pub(crate) async fn swarm_ai_assist(&self, parameters: Parameters<AiAssistRequest>) -> String {
        execute_tool_semantic(self, "swarm_ai_assist", Some(hkask_bridge_ontology::pko::PROCEDURE), async {
            ...
            // The inference port detects JSON-object tasks and
            ...
            // The previous `SkillExecPort` path (running the `swarm-compose-guide`
            // skill cascade) is removed — skill cascades are gone. The agent
            // calls the `skill` tool with `swarm-compose-guide` if it needs
