//! ABW cloud tools — split from `cloud_tools.rs` by capability group.
//!
//! This module is the home for the ABW (Agent Bestiary World) cloud-surface
//! tools. The original `cloud_tools.rs` was a 2076-line file holding 27 tool
...
//! `cloud_tools.rs` (the `#[tool_router]` impl) can stay focused on the
...
// Re-export the pure helpers so `cloud_tools.rs` and `test_utils` can import
