...
//! methods on `MemoryStore` (a SQLCipher-backed memory store). This module
...
/// Delegates to the pure implementation on `MemoryStore` (no DB access).
pub(crate) fn chunk_text(
    ...
    hkask_memory::MemoryStore::chunk_text(
        text,
        entity_ref_prefix,
        min_words,
        max_words,
...
pub(crate) fn strip_gutenberg_headers(text: &str) -> String {
    hkask_memory::MemoryStore::strip_gutenberg_headers(text)
}
