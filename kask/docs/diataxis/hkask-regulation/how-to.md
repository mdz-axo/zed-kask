...
| `RegulationPolicy::default` (rules) | `kask/crates/hkask-regulation/src/regulation_policy.rs:135` |
| `SetPoints` (set-point values) | `kask/crates/hkask-regulation/src/set_points.rs:184` |
...
verified_against: kask/crates/hkask-regulation/src/sensor_provider.rs:27,39,52,57,79,126; kask/crates/hkask-regulation/src/cybernetics_loop.rs:216-263; kask/crates/hkask-regulation/src/loops/signals.rs:14,245; kask/crates/hkask-regulation/src/regulation_policy.rs:135; kask/crates/hkask-regulation/src/set_points.rs:184
status: VERIFIED
-->
...
snake_case string to `as_str()` (`loops/signals.rs:114`). The string is used
...
`as_str()` mapping (`regulation_policy.rs:57`). Then add a `RegulationRule`
to `RegulationPolicy::default()` (`regulation_policy.rs:135`) that matches
the new metric and direction, with a `ProposedAction`
(`regulation_policy.rs:101`) naming the `ActionType` to propose. The
...
(`cybernetics_loop/cycle.rs:1636`).
...
The `Signal::new` constructor (`loops/signals.rs:254`) stamps the signal
with `chrono::Utc::now()`.

### Step 5: Register in `CyberneticsLoop::build`

In `CyberneticsLoop::build()` (`cybernetics_loop.rs:216`), inside the
`sensor_registry` block (`cybernetics_loop.rs:233-263`), add:
...
(`cybernetics_loop.rs:179`). `sense()` calls
`self.sensor_registry.sense_all(LoopId::Cybernetics)`
(`cybernetics_loop/cycle.rs:262`).

Also add the metric to the `SENSED` list in the blind-metric warn block
(`cybernetics_loop.rs:276-291`): the loop warns at startup about policy
...
Add the metric to `default_substitution_ladder` (`regulation_policy.rs:567`).
