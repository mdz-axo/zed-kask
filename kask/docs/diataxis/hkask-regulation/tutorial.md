...
| Symbol | Location |
|--------|----------|
| `CyberneticsLoop::tick` (cycle entry point) | `kask/crates/hkask-regulation/src/cybernetics_loop.rs:705` |
| `CyberneticsLoop::sense` | `kask/crates/hkask-regulation/src/cybernetics_loop/cycle.rs:254` |
| `CyberneticsLoop::compare` | `kask/crates/hkask-regulation/src/cybernetics_loop/cycle.rs:248` |
| `CyberneticsLoop::compute` | `kask/crates/hkask-regulation/src/cybernetics_loop/cycle.rs:290` |
| `CyberneticsLoop::act` | `kask/crates/hkask-regulation/src/cybernetics_loop/cycle.rs:348` |
...
| `RegulationPolicy::decide` (rule lookup) | `kask/crates/hkask-regulation/src/regulation_policy.rs:460` |
...
verified_against: kask/crates/hkask-regulation/src/cybernetics_loop.rs:705; kask/crates/hkask-regulation/src/cybernetics_loop/cycle.rs:254,248,290,348,662; kask/crates/hkask-regulation/src/runtime.rs:406,562
status: VERIFIED
-->

## Step 1: Sense — collect afferent signals

The cycle begins in `CyberneticsLoop::sense()` (`cycle.rs:254`). It first
drains the curator-directive inbox via `process_inbox()`
(`cybernetics_loop.rs:681`), so any `CuratorDirective` from the Curation
...
(`cycle.rs:269-277`) — when the log approaches its cap, an
...
(`cybernetics_loop.rs:233-263`), registers five sensors:
...
`with_*_health_source` builders (`cybernetics_loop.rs:466,504,542`). Each
...
`self.simulator.observe(signal.metric, signal.value)` (`cycle.rs:280-282`).

## Step 2: Compare — detect deviations

`CyberneticsLoop::compare()` (`cycle.rs:248`) is a one-liner: it filters
each `Signal` through `Deviation::from_signal` (`loops/signals.rs:274`),
...
`CyberneticsLoop::compute()` (`cycle.rs:290`) first runs the predictive
...
(`regulation_policy.rs:135`) via `decide()` (`regulation_policy.rs:460`),
...
`build_regulation_action` (`cycle.rs:1059`), which applies mode-specific
...
`CyberneticsLoop::act()` (`cycle.rs:348`) first calls `reset_all_caps()`
(`cybernetics_loop.rs:673`) — one regulation tick resets every agent's call
cap to its ceiling. It then detects call-cap exhaustion via
`CallCapManager::all_agent_statuses()` (`energy.rs:205`) and emits a
`Warning` alert for each exhausted agent (`cycle.rs:350-416`).

Each action is routed through `route_action_as_alert` (`cycle.rs:488`).
...
`CyberneticsLoop::verify_impact()` (`cycle.rs:662`) re-senses the metrics
...
Classification uses `classify_decision` (`regulation_policy.rs:544`) with
the `stage_worsening_ratio` (default 0.05) and `block_worsening_ratio`
(default 0.20) from `SetPoints` (`set_points.rs:106,112`).

When a `RolloutEventSource` is wired (`cybernetics_loop.rs:191`),
...
a `regulation_impact`-sourced verdict event (`cybernetics_loop.rs:69-106`).
Externally-submitted rollout checks (via `submit_rollout_impact_check`,
`cybernetics_loop.rs:593`) are drained into the current tick's verification
pass (`cybernetics_loop.rs:722-727`).
...
`loop_quality()` query (`cybernetics_loop.rs:874`). The cycle also emits a
`reg.runtime.select` telemetry span with the signal count
(`cybernetics_loop.rs:712-717`).
