...
semantics. This means each triple has two time axes: `valid_time` (when the
fact was true in the world) and `transaction_time` (when the fact was
recorded in the database). This design allows the system to answer both
"what did we know at time T?" and "what was true at time T?".
...
bitemporal model preserves both: the `valid_time` axis tracks when the fact
was true, and the `transaction_time` axis tracks when the database recorded
it.
