...
| Symbol | Location |
|--------|----------|
| Core schema loader | `kask/crates/hkask-storage/src/core/database.rs:200` |
| `initialize_schema` fn | `kask/crates/hkask-storage/src/core/database.rs:199` |
| `Database::open` | `kask/crates/hkask-storage/src/database/sqlite.rs:170` |
...
flowchart TD
    ...
    B -- store-specific --> D[Add CREATE TABLE to store's init_schema]
...
verified_against: kask/crates/hkask-storage/src/core/database.rs:199,200; kask/crates/hkask-storage/src/core/store_macros.rs:11; kask/crates/hkask-storage/src/regulation_store.rs:78
status: VERIFIED
-->
...
it belongs in `src/sql/schema.sql` (`core/database.rs:200`). If the table is
specific to one store (like `reg_records` for regulation or
`delegation_tokens` for the token registry), it belongs in that store's
`init_schema` method.
...
`core/database.rs:436` loads this file.

### Step 4: Add CRUD methods

Add methods to the store struct for inserting, querying, updating, and
deleting rows. The store struct holds an `Arc<dyn DatabaseDriver>` and calls
`driver.execute` or `driver.query` methods. Follow the pattern in
`regulation_store.rs` or `consent_store.rs`.

### Step 5: Add tests

Add tests in the store module or in a `tests/` subdirectory. The tests should
construct an in-memory database, call `init_schema`, and verify the CRUD
methods work. Run the tests with `cargo test -p hkask-storage`.

## See also

- [hkask-storage Reference](./reference.md): ERD of the full schema.
- [hkask-storage Tutorial](./tutorial.md): your first migration (planned).
