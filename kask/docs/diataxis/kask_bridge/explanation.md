...
24 top-level modules cover condensation, context injection, credentials,
database maintenance, delegation grants, identity, four inference adapters,
inference IPC/provider/socket support, MCP environment and registry wiring,
memory, model resolution, settings, metacognition, directives, algedonic and
...
executor (`kask/crates/kask_bridge/src/kask_bridge.rs:48-114`).
...
swarm-memory provisioning (`crates/zed/src/main.rs:1620-1624`).
...
(`kask/crates/kask_bridge/src/mcp_servers.rs:612-645,646-769`). The ordering is
...
(`kask/crates/kask_bridge/src/mcp_servers.rs:570-600,873-894`). This makes a
...
settings contain mode, URL, credit ceiling, curator-consent policy, skills path,
default-agent-model override, A2A toggle, and embedding dimension; they contain
no separate passphrase field (`kask/crates/kask_bridge/src/settings.rs:416-463`).
