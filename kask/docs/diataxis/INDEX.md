...
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------- |
| `hkask-mcp-companies`, `hkask-mcp-corpus`, `hkask-mcp-scenarios`, `hkask-mcp-curator`, `hkask-mcp-kata-kanban`, `hkask-mcp-research`, `hkask-mcp-swarm`, `hkask-mcp-training` | Already documented cross-cuttingly in `kask/docs/reference/mcp-servers/`. Note: `hkask-mcp-condenser`, `hkask-mcp-media`, and `hkask-mcp-codegraph` were deleted (2026-08-20). | [`reference/mcp-servers/README.md`](../reference/mcp-servers/README.md)                       |
| `hkask-forecast`, `hkask-email`, `hkask-ledger`, `hkask-keystore`, `hkask-memory`, `hkask-services-core`, `hkask-mcp`, `hkask-lisp`                                                                                                     | Small support crates (<3000 LOC); documented via cross-cutting docs (and crate READMEs where present — `hkask-email` has none). Note: the other `hkask-services-*` crates were folded into their MCP server consumers (F6 refactor). `hkask-guard` was deleted 2026-08-10 (the `RoleOverride` scanner's bare `system:` substring match blocked legitimate skill cascade template rendering); provider-side safety and refusal fallbacks remain. | [`architecture/zed-host-architecture-plan.md`](../architecture/zed-host-architecture-plan.md) |
...
- [`kask/docs/architecture/zed-host-architecture-plan.md`](../architecture/zed-host-architecture-plan.md): D1–D33 integration seams (D21 = conversation-injector, D22 = block-reachability pins, D23 = worktree spawn wiring; D24–D33 added 2026-08-05 through 2026-08-20 — see `DIVERGENCE.md` for the authoritative seam list).
- [`DIVERGENCE.md`](../../../DIVERGENCE.md): the authoritative divergence surface (D1–D33).
