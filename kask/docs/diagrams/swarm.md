...
(`hkask_mcp_swarm.rs:173-178`) — the former `ledger_router` was removed with
`hkask-ledger` and the local budget system (2026-09-08); `swarm_fleet_digest_local`
and `swarm_select_agent_local` were added 2026-09-09 with the fermi absorption
(`256f87307c`). The tool-name list is generated by `build.rs` from
`pub(crate) async fn swarm_*` signatures and asserted against the live
`combined_router()` surface at test time.
...
flowchart TD
    ...
    subgraph local[Local Substrate v2 S15]
        ...
        STATS[agent_stats<br/>per-agent execution stats]
...
classDiagram
    ...
    class LocalSwarmRuntime {
        ...
        -agent_stats: Arc~AgentStatsStore~
        +delegate(card,task) LocalDelegateResult
        measures the run no spending policy
    }
    class AgentExecutor {
