...
flowchart TD
    ...
    subgraph invocation["Invocation gate — fast clock"]
        ...
        I4["Rung 3: Grounding<br/>enforce_grounding"]
        I5{"Unsourced fields?"}
        I6["Null unsourced fields<br/>retain truncated preview<br/>scan narrative for leaks"]
        I7["Keep cleaned JSON<br/>log warn if nulled"]
        I8["Record result + comment"]
        I1 --> I2 --> I3 --> I4 --> I5
        I5 -- "Yes" --> I6 --> I7
        I5 -- "No" --> I7
        I7 --> I8
    ...
    subgraph vocabulary["Five-valued provenance"]
        V1["Sourced<br/>tool returned it → keep"]
        V2["Inferred<br/>commissioned judgment → keep"]
        V3["UncommissionedInference<br/>not commissioned but plausible → keep"]
        V4["Narrative<br/>prose → keep, scan"]
        V5["Unsourced<br/>no tool could supply → null"]
    end
...
verified_against: kask/mcp-servers/hkask-mcp-swarm/src/local_registry.rs (validate_presence, validate_typing); kask/mcp-servers/hkask-mcp-swarm/src/port_registry.rs (PortRegistry); kask/mcp-servers/hkask-mcp-swarm/src/local_runtime.rs (check_bind, classify_request); kask/mcp-servers/hkask-mcp-kata-kanban/src/grounding.rs (enforce_grounding, ProvenanceTag, task_agent_contract); kask/mcp-servers/hkask-mcp-kata-kanban/src/hkask_mcp_kata_kanban.rs (spawn_via_local_runtime grounding wiring, build_task_agent_card system prompt)
status: VERIFIED
-->
