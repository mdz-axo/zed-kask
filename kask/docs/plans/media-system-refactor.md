...
**Deferred (next slice):**
- **Durable gallery DB** (G14): `HKASK_MEDIA_DB` env support in the media server's `GalleryStore` construction. `Database::open(path, passphrase)` requires a passphrase decision — **do NOT reuse the global `HKASK_DB_PASSPHRASE`** (that leaks the curator/kanban decryption key to the media-server child process, violating the credential-blast-radius `.rules`); add a media-specific `HKASK_MEDIA_DB_PASSPHRASE` (or use an unencrypted file pool via `SqliteDriver::new(SqliteConnectionManager::file(path))` bypassing `Database`). Fall back to `Database::in_memory()` when unset/unwritable. Also persists `gallery_state` across restart (currently `Arc<Mutex<Option<GalleryState>>>` is in-memory). Add `HKASK_MEDIA_DB` (path) to the media `config_env` allowlist and the passphrase to `credentials`, and extend the `media_credentials_only_include_used_keys` test.
