...
**Status: APPLIED and VALIDATED (as-built diverges from the original design — simpler).** Instead of a new `BudgetLedger` in `hkask-inference/src/budget.rs`, the media server reuses the existing `hkask_templates::budget::BudgetTracker` (1 rJoule = $1 USD), used rJoule-only here: compute gas is enforced upstream at `McpRuntime::invoke` + `CyberneticsLoop`, so the tracker's gas cap is constructed inert (`hard_limit: false`, never charged) and the media server never charges gas. `MediaServer` holds `budget: MediaBudget` (resolved once at startup: an `Option<Arc<Mutex<BudgetTracker>>>` tracker + `UnitCosts` + `alert_threshold` + a one-shot `alerted` flag); `estimate_rjoule(&UnitCosts, tool, params)` is a pure function (no per-call env reads — unit costs resolve once from `HKASK_MEDIA_RJOULE_PER_IMAGE` / `_PER_TRANSFORM` / `_PER_UPSCALE` / `_PER_VIDEO_SECOND`) that computes a conservative pre-charge estimate; `charge_budget()` pre-charges and rejects with an `McpToolError` (propagated to the UI) when `remaining_rjoule() < estimate`. The gate is wired into the five billable generation tools (`generate_image`, `transform_image`, `upscale_image`, `generate_video`, `execute_workflow`) before `media_generate` — gating `execute_workflow` closes a bypass (its DAG can include otherwise-ungated billable nodes). `build_media_budget()` constructs the `MediaBudget` from `HKASK_MEDIA_RJOULE_CAP` (unset/0 ⇒ disabled; malformed ⇒ `tracing::warn!` + disabled, so the operator gets a config-error signal rather than a silently absent gate). `HKASK_MEDIA_RJOULE_ALERT_THRESHOLD` is enforced: `charge_budget_gate` fires a one-shot `tracing::warn!` when `used/cap >= threshold`. The duplicate `hkask-inference/src/budget.rs` (tracked USD, wrong unit) was removed. 8 `estimate_rjoule_tests` + 7 `charge_budget_gate_tests` (incl. `execute_workflow` floor, threshold-crossing, and below-threshold) pass; all 43 `hkask-mcp-media` lib tests pass (env-isolated — `estimate_rjoule` is pure of `UnitCosts`, no env reads); `./script/clippy -p hkask-mcp-media` clean under `--deny warnings`. The gate is check-before-charge under the `tokio::sync::Mutex`: a rejected request consumes no budget, and the mutex serializes concurrent bursts so they cannot all pass then overspend. **Deferred from the original design:** per-action approval thresholds, estimate/actual reconciliation, and the queryable ledger API — the as-built is a hard pre-charge gate, not a reserve/reconcile ledger.
