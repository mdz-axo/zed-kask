...
**R5 preregistration:** local record `local-r5-extractor-2026-09-18`, created **22:09:15 UTC before any generation**, prediction **0.60** that the candidate would satisfy the acceptance rule. Goal: improve exact grounded extraction without held-out regressions. Baseline is the system prompt from `/home/mdz-axolotl/Clones/zed-kask/agents/local/curated/local_extractor/agent_card.json`; candidate appends explicit instruction/data separation, verbatim-copy/no-normalization, requested-fields-only, missing-field accounting, and JSON-only instructions. Both prompts frozen before development/held-out samples; neither changed afterward. Baseline hash `e14021d7b09620c94a0df56d015b9b8742087d471ee43795d8a9ab6751`; candidate hash `6c6187f9ab4e1d371a011ca2110c9b36ecf9c77cfa5f8402de5f8d47617227da`.

**Route/bounds:** real freshly built `hkask-mcp-swarm` stdio child, isolated agent cards/data/event DB and environment with no cloud credentials, invokes actual `swarm_eval_agent_local`. Its OpenAI-compatible fallback is routed through a one-off loopback budget proxy to installed **local `qwen3.5:4b`** (4.7B Q4_K_M, no remote host/model). No tools/skills or original output-contract metadata were attached to either isolated card; only prompt differs. Proxy fixes temperature 0, seed 314159, think=false, context 4096, max output 256, 60-second per-call deadline, 16-call maximum, 12-minute whole-run cap. It rejects other endpoints/models and records raw Ollama responses. This adapter supplies bounds lacking in the production harness; it is not a production sandbox or spend-gate claim. No model download or paid provider call. Local energy cost unmeasured.

**Tasks/oracle:** two development and six held-out cases; one sample per arm/case, alternating arm order. Cases cover verbatim dates/Unicode/numbers, absent fields, extra unrequested data, and one instruction embedded in source text. Expected answers are never sent to inference. Oracle parses the entire trimmed output as JSON and compares exact key/value structure (object order irrelevant; array order significant); no fence stripping or answer-substring shortcuts. An anchored regex generated from the same expected object is supplied to the production harness, and its result is checked against the separate JSON oracle on every response. Oracle positive/negative controls run before generation. Held-out tasks were frozen but selected by the same engineer as the candidate: separated from model generation, not an independently staffed benchmark. No significance/generalization claim from this tiny single-model sample.

**Acceptance fixed before sampling:** complete valid evidence AND strictly more held-out successes AND zero baseline-success/candidate-failure held-out cases; otherwise reject/retain baseline. All 16 calls completed, finishing **22:10:23 UTC**. No post-result tuning or second candidate was attempted.

| Result | Baseline | Candidate |
| --- | --- | --- |
| Development exact JSON | 1/2 | 1/2 |
| Held-out exact JSON | **5/6** | **5/6** |
| Held-out gains / regressions | — | **0 / 0** |
| Prompt tokens, all eight cases | 1,286 | 1,966 |
| Output tokens, all eight cases | 231 | 234 |

**Decision: reject candidate; retain baseline.** Both variants mishandled an absent field in one mixed-present/missing held-out case. Candidate provides no measured accuracy gain and adds 680 prompt tokens (all cases). Elapsed inference times were 10.830s baseline/6.115s candidate but include unequal cold-load/cache conditions, so no speedup claim. Genuine provider spend is zero; local compute is not claimed free. Repository agent card and installed/active prompt remain unchanged; rejection is rollback-by-nonpromotion, not reversal of a deployed change.

**Durability/audit:** after stopping the isolated server, reopened its SQLite DB read-only and verified **16 model_request + 16 verdict + 16 harness_summary** events, with every verdict linked to a model_request and every returned run ID present. Startup logged database-locked warnings; all expected records survived reopen, but this is not proof of general crash recovery. Every report showed available capture and zero reported drops. Model name was checked on every raw response; weight digest `2a654d98e6fba55d452b7043684e9b57a947e393bbffa62485a7aac05ee4eefd` was captured **after** the run, not before, so no immutable-weight attestation is claimed. Preregistered prompts/tasks/driver hashes remained unchanged. Offline negative controls reject missing evidence, wrong model/artifact, regression, and no-gain selection; these are protocol checks, not R6 runtime enforcement.

**Artifacts:** `/home/mdz-axolotl/.local/state/zed-kask/verification/r5-cycle-2026-09-18/experiment/` contains read-only preregistration/prompts/tasks, `prerun-sha256.txt`, the one-off `run.cjs` driver, exact JSON oracle/control code, raw HTTP requests/Ollama responses, harness reports, isolated DB, verdict, audit, and negative controls. The driver is retained research evidence outside the repository, not a JavaScript project dependency or proposed production framework. Source diff/binary digest and final checksums accompany it. No `memory_insert`/kanban tool was available; durable local files are used under the approved exception, not invented h_mem IDs.

**Closure state:** R5's genuine baseline/candidate evaluation and rejection traversal executed. **Operator confirmation pending; Brier score remains null.** On confirmation that the predeclared improvement criterion was not achieved, the recorded 0.60 prediction yields Brier `(0.60 - 0)^2 = 0.36`; do not store it as confirmed before operator ground truth. This is evidence that the loop can reject an unhelpful change, not evidence of beneficial self-rewriting. Preserve the baseline, archive the candidate, and require a new preregistration/data-exposure accounting before another candidate. R6 authority/evidence-binding/recovery remains separate unfinished work.
