...
| 6 | Runtime monitoring | **Present** | `with_wallet()` attaches balance to every spend response; `tracing::warn!` on stale signals (wallet, cost, consent); `detect_embedded_error` catches ABW-wrapped LLM failures; `hkask.mcp.swarm` telemetry targets |
