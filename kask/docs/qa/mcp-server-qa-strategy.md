...
| 7 | `hkask-mcp-media` | `hkask-mcp-media` | stdio | `src/hkask_mcp_media.rs` | `run()` | **optional**: `DEEPINFRA_API_KEY`, `FALAI_API_KEY` (vision via IPC bridge to zed); **budget**: `HKASK_MEDIA_RJOULE_CAP` (rJoule/USD ceiling; unset = disabled), `HKASK_MEDIA_RJOULE_PER_*` (unit-cost estimates) |
