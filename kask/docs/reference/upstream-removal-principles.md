...
> with a test (`.rules:248-276`). Every principle below is *compatible* with
> this: no principle authorizes forking upstream outside a D-seam. For upstream
...
- **Decision test (falsifiable):** Does the retained upstream surface cause a
  ...
  `.desktop` template rendered for zed-kask.
...
- **Anchoring evidence:** `.rules:787-821` (the `.desktop` collision trap);
  `DIVERGENCE.md` D7 L28 (Hard Zed-isolation invariant: "upstream Zed's
  updater is not initialized or imported by the zed binary"; legacy bundlers
  "fail-closed"; enforced by `check-zed-isolation.sh`); `DIVERGENCE.md` D16
...
- **Anchoring evidence:** `DIVERGENCE.md` D7 L28: "Legacy Zed bundlers for
  ...
  `script/bundle-linux` is the active bundler.
...
- **Failure mode if mis-applied:** (a) removing surface that *appears* dead
  ...
  "LLM-improves-against-LLM-scored-target" gaming (`.rules:835-851`): an agent
  declares surface "dead" to reduce a merge-friction score without genuinely
  verifying G1.
- **Anchoring evidence:** `.rules:586-602` (Trait-with-one-impl is speculative
...
- **Failure mode if mis-applied:** removing upstream surface that kask
  *appears* to replace but the kask replacement is unwired (the
  "advertised invariants need enforcement points" trap, `.rules:602-631`) →
  silent loss of functionality with no error. This is the highest-risk
...
forking upstream outside a D-seam ("renaming, reformatting, or 'correcting'
upstream files creates merge noise and risks silent breakage on the next
upstream merge," `.rules:253-255`). The `.rules` vim-test JSONL example
(`.rules:261-265`: 307 upstream files, "leave them alone") is the canonical
counter-example. Only Category 4's *provably unreachable* test survives.
...
| --- | --- | --- | --- |
| C1 Collision | Only on a **D-seam file** (`crates/zed/resources/zed.desktop.in` is a D7 seam; menu files are D16 seams). New collision surface → new D-seam entry in `DIVERGENCE.md` + test pin (`.rules:256-259`). | disable-behind-D-seam + `check-*-no-collision.sh` | ✅ compatible |
...
| C3 Redundant | All current instances are D-seam files (D1 `agent.rs`/`agent_skills.rs`; D3 `main.rs`; D16 menu files). New redundant surface → new D-seam entry + test pin (`.rules:267-274`). | disable-behind-D-seam + test pin | ✅ compatible |
...
entry in `DIVERGENCE.md` pinned by a test in the same PR (`.rules:256-259`).
...
| C1 × C3 | yes | D16 upstream update actions removed (collision *and* replaced by `RunZedKaskUpdate`) | **C1** — the installer-replaces-real-Zed risk is the load-bearing reason; C3 (redundancy) is co-incident. |
...
Ground-truth prior-removal set, drawn from `DIVERGENCE.md` D1–D24 + `.rules`
traps (verified against the codebase per `.rules:464-475`):

| # | Prior removal | Source | Classified under |
| --- | --- | --- | --- |
| 1 | Upstream auto-update not initialized/imported | D7 L28 | C1 |
| 2 | Legacy bundlers macOS/Windows/Snap fail-closed/deleted | D7 L28 | C2 |
| 3 | macOS `osx_url_schemes = ["zed"]` + bundle sections removed | D7 L28 | C1+C2 (co-occurrence) |
| 4 | Flatpak/Snap resources deleted | D7 L28 | C2 |
| 5 | Upstream release workflows + Zed desktop template deleted | D7 L28 | C2 |
| 6 | Catalog budget disabled | D1 L22; `agent.rs:4224` | C3 |
| 7 | Description-length warnings disabled | D1 L22; `agent_skills.rs:401` | C3 |
...
(12/12 classifiable) AND C4 carries an explicit "no upstream anchor —
...
Rationale for 0.70 (not higher): C1/C2/C3 are well-anchored and cover all 12
...
truth. Per `.rules:202-208` ("no drive-by additions"; rules must be
"repeatedly encountered"), a category with zero instances is not added; it is
recorded as an open question.

### Experiment (Kata Step 4)

Applied the taxonomy to the 12-case ground truth (table above). Every case is
...
- **Brier score:** predicted p = 0.70 that the taxonomy covers the next 5.
  ...
  (12/12 classifiable). **Brier = (0.70 − 1.0)² = 0.09** against the proxy.
...
2. **Possible 5th category — security/license removal.** No ground-truth
   ...
   `.rules:202-208`). If a real upstream CVE or license-incompatibility
...
- [x] Every citation is a real file:line (verified: `.rules:787-821`,
      `DIVERGENCE.md` D7 L28 / D16 L37 / D1 L22 / D3 L24,
