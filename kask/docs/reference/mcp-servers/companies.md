...
- Cache parameter keys are prefixed `normalized-v1:`. Each value contains normalized data, its actual provider and warnings, all retained across reopen. Old raw-only rows are ignored without deleting/migrating the database. Endpoint TTLs and opportunistic concept extraction remain unchanged; concept-cache lifecycle decisions are excluded.
...
Offline regression coverage: `src/acquisition_tests.rs`, using loopback HTTP through actual provider request/normalization paths. Run with provider keys removed; live schema checks are separate (see the crate README).
