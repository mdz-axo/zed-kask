...
#### Acquisition representation (operator decision, 2026-09-06)

The approved acquisition/valuation-preparation slice preserves `160cef9fab` (typed views/testable valuation), `43e28cc484` (two-layer cache) and `b8057bb2f4` (visible missing data). It supersedes only the ambiguous raw/enriched cache representation and duplicated DCF preparation:

- `key_metrics` returns `content.data` (normalized annual rows), `content.provider` (`FMP` or `EODHD`) and `content.warnings`, plus ontology. This replaces the former bare-array content; no backward-compatible array alias is provided.
- FMP enrichment joins ratios and financial growth by date and never silently ignores failed/malformed/unmatched supplements. EODHD annual metrics use annual financial-statement dates, retain available matching earnings fields, and do not mix in FMP values. Existing EODHD approximations are explicitly warned; missing net debt does not imply a zero-debt EV multiple.
- Cache parameter keys are prefixed `normalized-v2:`. Each value contains normalized data, its actual provider and warnings, all retained across reopen. Old raw-only and pre-sanitizer `normalized-v1:` rows are never read or logged through acquisition; the next request fetches fresh data once. Old bytes remain on disk without deleting/migrating the database. Endpoint TTLs and opportunistic concept extraction remain unchanged; concept-cache lifecycle decisions are excluded.
- Comparable targets and concurrent peers use the same server cache/learning policy. Rows carry endpoint `provenance`, `warnings` and `errors`; failed peers remain identified, and missing multiples/prices are omitted rather than zero-valued.
- Standalone DCF and comparable overlays share `valuation_service::prepare_dcf`: typed profile/history, financial-sector guard, explicit missing price/shares/history handling, defaults and overrides. With identical inputs and common assumptions their intrinsic value, current price and margin of safety agree. Provider outages appear as overlay errors without hiding the comparison table; invalid assumptions remain typed errors. Standalone persistence and distinct EP/Monte Carlo/other engines are unchanged.
...
- `action: calculate` selects `template` plus a schema-visible typed `template_context`, or accepts the same canonical document through `screen_definition`. Before Jinja2-compatible rendering, the server stamps `as_of` from the current UTC acquisition date, validates caller context against `[inference] contract.input` and server context against `contract.server_input`, and returns every missing field in one structured `invalid_argument`; strict undefined handling remains a backstop. Direct definitions must carry the same current date. At least one exchange is required, preventing historical mislabeling and unrestricted fallback.
...
- The calculate request persists and returns a `queued` job before provider acquisition. The background job freezes one EODHD listing result set, executes derived columns, and persists `executing`, `completed`, or `failed` state. A provider error or unwind panic becomes `failed`; a missing job row surfaces as a lost transition rather than false success. Because the frozen universe is process-local until completion, server startup marks interrupted `queued` or `executing` jobs `failed` with an explicit resubmission reason rather than reporting false liveness.
...
- The `expectations_gap` template filters strict USD issuer capitalization and computes USD liquidity as `adjusted_close × avgvol_200d × listing-unit adjustment ÷ listing-currency-per-USD` before acquiring fundamentals. It joins EODHD's active common-stock ticker master and groups qualifying lines through connected LEI, primary-ticker, ISIN, and non-conflicting normalized-name evidence. The canonical issuer key follows LEI → primary ticker → ISIN → normalized-name fallback precedence; output surfaces key and grouping provenance, all eligible lines, analysis/actionable line selection, and price/currency-normalization provenance. For non-financials, the primary growth gap is implied revenue growth minus demonstrated full-period revenue CAGR; implied growth minus Higgins SGR is a separate financing-headroom diagnostic. Demonstrated DuPont ROE and turnover use average beginning/ending balance-sheet denominators, and the reverse DCF deducts normalized SG&A. The uncalibrated composite score is withheld; raw dimensions and model-sensitivity flags are returned without a synthetic ranking. Company research remains downstream of the screen.
...
- **Ontology anchoring.** Every tool output carries a top-level `"ontology"` key with the artifact's concept URI. Tools whose concept FIBO actually publishes anchor on verified FIBO URIs (e.g. `fibo-be-le-cb:Corporation`, `fibo-sec-sec-id:TickerSymbol`, `fibo-ind-mkt-bas:MarketCapitalization`); analysis-family tools with no FIBO equivalent anchor on Dublin Core (`bibo:Report` for analysis outputs, `dcmitype:Dataset` for data outputs) per the operator decision 2026-08-29. Per-field metric metadata uses the `"metric"` key with hKask-internal canonical metric names (e.g. `revenue_growth_rate`) — not ontology URIs; FIBO publishes no terms for financial ratios (verified against the FIBO master ontology 2026-08-29). Raw provider payloads are returned without a metric mapping. The `"ontology"` field is the unified dispatch concept (see [Ontology Bridge](../ontology-bridge.md#unified-ontology-tag-shape)).
