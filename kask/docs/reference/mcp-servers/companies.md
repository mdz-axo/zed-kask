...
- **Governance is at the dispatcher membrane.** OCAP is enforced by the `McpRuntime::invoke` / `ToolGovernance` membrane in `crates/hkask-mcp/src/runtime.rs`, which verifies a `DelegationToken` per call (signature + authority) before the request reaches this server. The companies server is the transport pipe; it does not re-check capabilities per call.
