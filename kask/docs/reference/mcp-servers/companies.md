...
| Component | Role |
|-----------|------|
| `CompaniesServer` | Server struct: `webid`, `client`, FMP/EODHD keys, optional research keys (Exa/Tavily/Brave), `ResearchStore`, `LearningState` (`Arc<Mutex>`), `FermiDefaults`, and the configured investor required return. Generated by `mcp_server!` (no `userpod`, no `daemon` field). |
...
flowchart TD
    ...
    Boot --> Srv["CompaniesServer<br/>learning research store"]
    ...
    Comb --> R3["valuation_router<br/>11 tools"]
    Comb --> R4["notes_router<br/>6 tools"]
    Comb --> R5["analytics_router<br/>3 tools"]
...
- `calculate` persists and returns a queued job before acquisition. Expectations-gap jobs checkpoint the deterministic security pass set and one work item per provisional issuer before financial enrichment. The pass stage is bounded to 60 seconds; enrichment is rate-limited, concurrent, and bounded to 110 seconds so report finalization remains inside the three-minute target.
- `status` exposes stage, processed/total, complete/partial/unavailable/model-sensitive counts, heartbeat and heartbeat age, elapsed time, ETA, cancellation state, and the canonical artifact path. Heartbeats persist every two seconds. `cancel` records cancellation immediately; active acquisition and enrichment observe the durable flag. Startup requeues interrupted work and resumes completed checkpoints without reacquiring the universe.
