...
- The calculation freezes one EODHD listing result set, executes derived columns as a background job, and persists `queued`, `executing`, `completed`, or `failed` state. Because the frozen universe is process-local until completion, server startup marks interrupted `queued` or `executing` jobs `failed` with an explicit resubmission reason rather than reporting false liveness.
