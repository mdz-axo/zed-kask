...
- `action: calculate` selects `template` plus a schema-visible typed `template_context`, or accepts the same canonical document through `screen_definition`. Before Jinja2-compatible rendering, the server validates context presence against the template's declared `[inference] contract.input` and returns every missing field in one structured `invalid_argument`; strict undefined handling remains a backstop. Both paths deserialize to one typed definition. Current-only acquisition requires `as_of` to equal the current UTC date and requires at least one exchange, preventing historical mislabeling and unrestricted fallback.
...
- The calculate request currently freezes one EODHD listing result set before returning, then executes derived columns as a background job and persists `queued`, `executing`, `completed`, or `failed` state. A returned error or unwind panic becomes `failed`; a missing job row surfaces as a lost transition rather than false success. Because the frozen universe is process-local until completion, server startup marks interrupted `queued` or `executing` jobs `failed` with an explicit resubmission reason rather than reporting false liveness.
