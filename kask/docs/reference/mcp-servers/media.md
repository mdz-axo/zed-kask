...
**Environment variables** (all optional unless noted; defaults resolve from `hkask_inference::model_constants`, `src/hkask_mcp_media.rs:64-103`):

| Variable | Purpose | Default source |
|----------|---------|-----------------|
| `HKASK_INFERENCE_SOCKET` | IPC bridge socket — routes vision/chat/media generation through zed's `LanguageModelRegistry` (`kask/crates/kask_bridge/src/mcp_servers.rs:415-417`) | — |
| `HKASK_DATA_DIR` | Data dir for gallery DB path resolution (`kask/crates/kask_bridge/src/mcp_servers.rs:418-419`) | — |
| `HKASK_MEDIA_DB` | Gallery DB path override (`src/hkask_mcp_media.rs:485`) | `{data_dir}/mcp/media/gallery.db` |
| `HKASK_MEDIA_TTS_MODEL` | TTS model (`src/hkask_mcp_media.rs:71-72`) | `DEFAULT_TTS_MODEL` (Qwen3-TTS via fal.ai) |
| `HKASK_MEDIA_STT_MODEL` | STT model (`src/hkask_mcp_media.rs:74-75`) | `DEFAULT_STT_MODEL` (fal.ai Wizper) |
| `HKASK_MEDIA_VISION_MODEL` | Vision model (`src/hkask_mcp_media.rs:78-79`) | `DEFAULT_VISION_MODEL` (Qwen3-VL via OpenRouter) |
| `HKASK_MEDIA_IMAGE_GEN_MODEL` | Image generation model (`src/hkask_mcp_media.rs:82-83`) | `DEFAULT_IMAGE_GEN_MODEL` (FLUX.2 [dev] via fal.ai) |
| `HKASK_MEDIA_VIDEO_MODEL` | Video generation model (`src/tools/models.rs:19-22`) | `DEFAULT_VIDEO_MODEL` |
| `HKASK_EMBEDDING_MODEL` | Embedding model for gallery similarity search (`src/hkask_mcp_media.rs:286-290`; resolver at `hkask-inference/src/model_constants.rs:78-80`) | `ollama/nomic-embed-text` |
...
| `omc:MediaSource` | `generate_speech`, `audio_capture`, `record_and_transcribe`, `voice_design`, `transcribe`, `transcribe_bundle`, `audio_trim`, `audio_concat` |
...
- **Face pipeline:** `face_validate` → `face_register` (or `face_scan_folder` in bulk) → `gallery_refresh` with `include_faces=true` → `gallery_name_face` → `gallery_search` by person name
