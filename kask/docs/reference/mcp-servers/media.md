...
Tool count is pinned against the live router; the generated tool-name set is checked against that same surface in `kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:486-514`. The canonical tool-name list is code-generated by `kask/mcp-servers/hkask-mcp-media/build.rs` into `tool_names.gen.rs` and included at `kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:455`; `tool_names_match_live_router` pins it to the live router at `kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:496-513`.

## Architecture

The server is a thin binary over a library: `kask/mcp-servers/hkask-mcp-media/src/main.rs:6-9` calls `hkask_mcp_media::run()`, which resolves the inference port, opens the gallery DB, and hands a `MediaServer` constructor closure to `hkask_mcp_server::run_server` (`kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:574-686`). The single `InferencePort` separates two routes (D35): vision/chat/embed use the IPC bridge to zed's `LanguageModelRegistry`; `media_generate` dispatches child-locally through `LazyInferencePort` to the media process's `MediaRouter` using env-injected provider keys. Media generation never makes the IPC round-trip.

```mermaid
flowchart TD
    binary["main.rs binary<br/>#[tokio::main] → run()"]
    run["run()<br/>hkask_mcp_media.rs:574"]
    ...
    server["MediaServer<br/>9 state fields"]
    router["combined_router<br/>10 sub-routers, 97 tools"]
...
verified_against: kask/mcp-servers/hkask-mcp-media/src/main.rs:6-9; kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:155-175,455-472,574-686; kask/mcp-servers/hkask-mcp-media/src/tools.rs
status: VERIFIED
-->

### Server struct

`MediaServer` is declared via the `mcp_server!` macro with nine state fields (`kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:153-180`):
...
| `ffmpeg: FfmpegRunner` | ffmpeg availability probe (`kask/mcp-servers/hkask-mcp-media/src/video/ffmpeg.rs:128`) |
...
| `reduct_api_key: Option<String>` | Reduct cloud API credential; not echoed in tool output or logs |

### Router composition

`combined_router()` sums ten sub-routers, one per registered `tools/` module: `gallery_router` + `processing_router` + `audio_router` + `generation_router` + `models_router` + `jobs_router` + `workflows_router` + `educt_router` + `youtube_router` + `reduct_router`. The handler attribute wires that sum as the runtime surface (`kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:458-472`).

## Bootstrap and configuration

**Bootstrap pattern:** `kask/mcp-servers/hkask-mcp-media/src/main.rs:6-9` (`#[tokio::main] async fn main` → `hkask_mcp_media::run()`) → `run()` at `kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:574` → `hkask_mcp_server::run_server("hkask-mcp-media", env!("CARGO_PKG_VERSION"), ...)` at `kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:652-685`.

**Gallery DB:** durable file-backed SQLite at `{kask_data_dir}/mcp/media/gallery.db` (D28 — Standardized Artifact Storage), overridable via `HKASK_MEDIA_DB`. There is **no in-memory fallback**: a DB open failure aborts startup, because the fallback would make an outage look like an empty gallery. The file DB is unencrypted and therefore does **not** use `HKASK_DB_PASSPHRASE` (`kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:587-631`).
...
**Credentials:** the media server's registration allowlists `OPENROUTER_API_KEY`, `DEEPINFRA_API_KEY`, `HKASK_SERPAPI_API_KEY`, and `REDUCT_API_KEY` (`kask/crates/kask_bridge/src/mcp_servers.rs:493-499`). The first two serve the child-local `MediaRouter`; SerpApi supplies structured YouTube discovery metadata. Vision/chat/embed use the IPC bridge to zed's `LanguageModelRegistry`. The routes are distinct — `yt-dlp` is not a metadata provider and needs no credential.

**Environment variables** (all optional; the five `HKASK_MEDIA_*_MODEL` vars resolve through the server's `models` module — `None` when unset, no constant fallback; callers fail visibly naming the env var, `kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:94-122`):
...
| `HKASK_MEDIA_DB` | Gallery DB path override (`kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:602`) | `{data_dir}/mcp/media/gallery.db` |
...
| `HKASK_EMBEDDING_MODEL` | Embedding model for gallery similarity search (`kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:397-410`) | not configured — embedding-dependent calls fail visibly naming `kask.models.embedding_model` |
...
The kask settings UI can populate the five `HKASK_MEDIA_*_MODEL` overrides (TTS, STT, vision, image-gen, video) via `KaskMediaSettings` → `emit_media_env` (`kask/crates/kask_bridge/src/mcp_env.rs:373`; wired at `kask/crates/kask_bridge/src/settings.rs:767`). The STT model also serves the educt transcript passes and `voice_design`.

**System dependencies:**

- **ffmpeg** — required by all local video tools and `audio_trim`/`audio_concat`. `require_ffmpeg` returns `McpToolError::unavailable("ffmpeg not found on system PATH — video tools unavailable.")` when absent (`kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:359-367`). `video_info` additionally uses ffprobe, bundled with ffmpeg (`kask/mcp-servers/hkask-mcp-media/src/tools/processing.rs:1299` tool description).
...
- **Vision-capable provider** — required by describe/analyze/caption/expand-prompt tools. `require_vision` returns `permission_denied` directing the operator to enable a vision model in the kask inference provider settings (`kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:382-392`).
...
credential-status/probe, bounded project/recording/reel snapshots, recording
status, JSON/TXT transcript, provider-native highlight retrieval and reel-detail
read with recursive share-token removal. Project, recording-status, JSON/TXT
transcript, highlight and reel-detail GET paths passed explicit live tests using
the stored key; key and response content were not printed in those tests.
...
The operator's 55-page v3 PDF supplies the previously missing write schemas:
...
remain unsupported. See the media server README for the operation boundary.
...
| `gallery_delete_image` | Delete an Asset from the gallery index by stable `image_id` or active `image_index` (exactly one); linked transcripts detach and survive. Index-only deletion preserves source bytes in every mode. `delete_file=true` requires destructive gallery mode; read-only/copy-on-write refuse before mutation. Unlink failure preserves the index/link; a later database failure explicitly reports that the source was already deleted and requires reconciliation (`src/tools/gallery.rs:1413–1444`; pinned by `gallery_deletion_mode_matrix` and `asset_file_deletion_failure_preserves_gallery_and_transcript_identity`). |
| `gallery_add_media` | Import a video or audio file into the gallery whose folder contains it (not the active gallery; a file outside every gallery folder is refused) (media_type selects the kind); SHA-256 hash for deduplication. The former `gallery_add_video`/`gallery_add_audio` pair, merged. |
...
| `omc:Asset` | gallery management + retrieval (`gallery_search` … `youtube_search` and `video_fetch`), all face tools — faces are gallery assets (people identified within images), not OMC `Participant`, which is a production-side concept about who made the media (`kask/mcp-servers/hkask-mcp-media/src/omc.rs:113-133`) |
