...
`combined_router()` sums nine sub-routers, one per registered `tools/` module: `gallery_router` + `processing_router` + `audio_router` + `generation_router` + `models_router` + `jobs_router` + `workflows_router` + `educt_router` + `youtube_router`. The handler attribute wires that sum as the runtime surface (`kask/mcp-servers/hkask-mcp-media/src/hkask_mcp_media.rs:458-472`).
