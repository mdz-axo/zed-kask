...
**Tools:** 12 — `market_lookup`, `market_match`, `market_ontology_map`, `market_calibration`, `market_record_resolution`, `market_subscribe_resolutions`, `market_ladder`, `market_cmp`, `market_cmp_index`, `market_residual`, `market_check_resolutions`, `market_history`, plus `prediction_markets_status`
**Auto-start:** No (requires explicit opt-in via KaskSettings toggle (D9a))

> **Tool count note:** the server registers **12 market tools** plus a
> `prediction_markets_status` state tool (13 `#[tool]` methods total in
> `src/hkask_mcp_prediction_markets.rs`). This reference catalogues the 12
