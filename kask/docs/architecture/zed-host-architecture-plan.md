...
- [§3 — The Minimal Divergence Map (D1–D38)](#3-the-minimal-divergence-map-exact-zed-kask-touch-points)
- [§4 — (removed)](#4-removed)
- [§5 — (removed)](#5-removed)
...
- [§13 — Composition & Connection Surfaces](#13-composition--connection-surfaces-zoom-out-review)
  ...
  - [§13.4 — Consolidated divergence map (D1–D38)](#134-consolidated-divergence-map-d1d38)
- [§14 — Repository Consolidation](#14-repository-consolidation--full-merge-into-zed-kask)
- [References](#references)

---

> **Current state (2026-08-28):** The kask workspace has **18 kask crates** under `kask/crates/` (17 `hkask-*` + `kask_bridge`) plus **11 MCP server crates** under `kask/mcp-servers/` and zed-side crates (`crates/swarm_panel/`, `crates/kanban_panel/`, `crates/portfolio_panel/`, `crates/hkask-steer/`, `crates/hkask-viz-core/`, `crates/hkask-*-widget/`, `crates/hkask-tool-invoker/`, `crates/hkask-conversation-injector/`, `crates/marketplace_ui_common/`). The `kask_extensions_ui` crate was removed 2026-08-20 (skill marketplace retired). D1–D38 are wired at the composition root (`crates/zed/src/main.rs`); the authoritative divergence surface is [`DIVERGENCE.md`](../../../DIVERGENCE.md) at the repo root. The composition-root wiring is documented in [§13.3](#133-composition-root-startup--di-pattern).
...
**Status (2026-08-28):** workspace builds clean. **18 kask crates** under `kask/crates/` (17 `hkask-*` + `kask_bridge`) plus **11 MCP server crates** under `kask/mcp-servers/` and zed-side crates (`swarm_panel`, `kanban_panel`, `portfolio_panel`, `hkask-steer`, `hkask-viz-core`, `hkask-*-widget`, `hkask-tool-invoker`, `hkask-conversation-injector`, `marketplace_ui_common`). The `kask_extensions_ui` crate was removed 2026-08-20 (skill marketplace retired). 11 MCP servers on disk (curator may be unloaded via `kask.mcp.overrides`).

| Crate                                                                                     | Why irreducible                                                                                                                                                                                                                                                                                                            |
| ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `hkask-types`                                                                             | Foundation: IDs, `InferencePort` trait (incl. `generate_batch` D34, `media_generate` D35), `MemoryPort`, `RegulationSpan`, `CuratorDirective`, vocab. `VoiceDesign`, `ExpectProposal`, and `HMemEntry` live here (consolidated from removed crates).                                                                                                                                           |
| `hkask-storage`                                                                           | **Sovereignty:** per-user/curator data directory encrypted private sphere (P11.1). Dual-backend: SQLite (SQLCipher, default) or PostgreSQL (pgvector, for scale-up). Hosts the media `GalleryStore` (D35).                                                                                                                                                       |
| `hkask-memory`                                                                            | Unique memory + confidence-based consolidation.                                                                                                                                                                                                                                                                           |
| `hkask-regulation`                                                                        | Cybernetic nervous system (`reg.*`, variety, algedonic, set-points). Per-agent governed tool calls are bounded by `CallCapManager` (1 call charged per `McpRuntime::invoke`, resets per tick). 18 source files / 9,944 lines (measured 2026-08-28).                      |
| `hkask-tool-port`                                                                        | **`ToolPort` dispatch seam** (`src/tool_port.rs:89`). Not an enforcement point: it holds no tokens, no authorization check (RR-0056), and no taint labels (RR-0053).                                                                                                                                                                                                                                                                       |
| `hkask-keystore`                                                                          | **Sovereignty crypto only:** DB passphrase, internal-secret derivation w/ versioning. Uses the `keyring` crate directly for all keychain access (no `SecretsPort` trait). `DEFAULT_PASSPHRASE` ("allostery") is the single source of truth for first-run provisioning.                                                        |
| `hkask-ledger`                                                                            | hMem accounting.                                                                                                                                                                                                                                                                                                           |
| `hkask-event-store`                                                                       | Shared event store: harness-summary events feed the CyberneticsLoop's rollout-impact checks (`kask_bridge/src/rollout_event_bridge.rs`, `hkask-mcp-training` rollout bridge).                                                                                                                                              |
| `hkask-inference`                                                                         | **Kept (revised):** MCP servers use it directly (`MediaRouter` D35, batch routing D34, `InferenceIpcClient`, `ProviderId`). Reads API keys from the `keyring` crate directly. Embeddings are handled by `kask_bridge::LanguageModelEmbeddingPort` (resolves credentials from `INFERENCE_PROVIDERS` + env var, no `LanguageModelRegistry` lookup). |
...
| D29–D38 | (See §13.4 consolidated map)                               | —                                                                                                                                                                                   | ✅ DONE    | RunPod provider (D29), upstream bug fixes (D31, D32), swarm panel CRUD (D33), batch inference (D34), media server (D35), `finish_reason: "stop"` tool-call drain (D36), commit-message reasoning guard (D37), thread-title reasoning guard (D38). Enumerated authoritatively in §13.4 and `DIVERGENCE.md`.                                                                                                                                                                                                                   |

**Discipline:** D1–D38 (D4 and D10 removed; D17 and D19 retired; D30 absent) are the _only_ edits to zed-kask's tree outside `kask/`. Any hKask behavior that would require touching other Zed crates is a smell — push the logic into an hKask crate behind one of these seams instead.

---

## 4. (removed)

> The phased migration-plan subsection that previously occupied §4–§5 has been removed. All phases are complete: D1–D38 are wired (see §3 divergence map). The `DIVERGENCE.md` at the repo root is the authoritative record of the divergence surface.[^fowler-strangler]
...
> The phased migration plan that previously occupied this section has been removed. All phases are complete: D1–D38 are wired (see §3 divergence map). The `DIVERGENCE.md` at the repo root is the authoritative record of the divergence surface.[^fowler-strangler]
...
| `keyring` crate (synchronous OS keychain)           | `hkask-keystore` (sovereignty keys only — DB passphrase, SQLCipher)                                                                                                                       | sovereignty keys (D5) — NOT routed through zed's `CredentialsProvider`  | D5     |
...
3. **Construct hKask components directly:** per-user/curator data directory storage (SQLite SQLCipher or PostgreSQL via `ServiceConfig::open_driver()`), Regulation runtime, memory, the native Curator agent (D2), the 11 MCP servers (standalone, identity from `ServerContext.webid` resolved out of `HKASK_WEBID`).
...
5. **Wire the regulation system:** construct `RegulationLedger::with_set_points(...)` (`crates/zed/src/main.rs:772-777`) + `CyberneticsLoop::with_set_points(ledger, set_points)` with alert + `CuratorDirective` channels (`main.rs:786-793`) + `NoopEventSink` (`main.rs:628-629`), seed the `swarm-panel` persona `CallCap` (`main.rs:849-855`), and call `McpRuntime::with_governance(loop, sink)` (`main.rs:880-884`). Startup log: "hKask regulation system wired — tool invocations are governed, regulation spans forwarded to ledger subscribers" (`main.rs:882-883`). `hkask-regulation` and `tokio` are dependencies of `zed`.
...
8. **Deferred agent provisioning (D6 late):** after `AppState::set_global`, a spawned task watches `UserStore::current_user()`. When the Zed user resolves: `provision_agent(username)` (`kask_bridge/src/identity.rs:112`) creates the directory structure, ensures a DB passphrase (defaults to `"allostery"` via `hkask_keystore::passphrase::DEFAULT_PASSPHRASE`, `identity.rs:161-171` — the user can change it later via the settings UI Security page, which triggers atomic DB rotation), and calls `set_memory_port(Some(bridge))` (`crates/zed/src/main.rs:1691`) to replace the `None` port. **First-run DB-passphrase provisioning at MCP launch:** `provision_db_passphrase` (`identity.rs:145`) is invoked from the MCP launch path (`mcp_servers.rs:676-677`) so child servers always find a passphrase even before the user resolves. (The former `mirror_provisioned_db_passphrase` bridge no longer exists.) MCP servers are launched without a per-user `HKASK_WEBID`; they fall back to anonymous identity unless the operator sets `HKASK_WEBID` in the environment.
...
### 13.4 Consolidated divergence map (D1–D38)
...
└── kask/              # ── OURS (additive; upstream never touches here) ──
    ...
    │                  # hkask-tool-port, hkask-keystore, hkask-ledger,
    │                  # hkask-event-store, hkask-mcp, hkask-mcp-server,
    │                  # hkask-inference, hkask-condenser, hkask-forecast,
    │                  # hkask-bridge-ontology, hkask-services-core,
    │                  # hkask-email, hkask-lisp,
    │                  # kask_bridge (D8)
    ├── mcp-servers/   # the 11 on-disk servers (curator may be unloaded via override; hkask-mcp-*)
    ├── skills/        # the skills registry (65 SKILL.md files in .agents/skills/;
    │                  # 61 template directories under kask/registry/templates/)
...
All Kask documentation lives **inside zed-kask** under `kask/docs/` (condensed 2026-08-28 to 69 documents; the former `plans/`, `explanation/`, and `qa/` directories no longer exist):
...
Former `plans/` and `explanation/` content was repointed: swarm system docs → `kask/docs/diataxis/swarm_system/`, memory → `kask/docs/architecture/memory-system-specification.md`, forecasting → `kask/docs/reference/mcp-servers/README.md#the-forecasting-stack-three-layer-architecture`.
