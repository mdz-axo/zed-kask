...
**P8.2 — Agent Output Grounding (v0.35.0):** LLM-produced agent output is grounded against a field → tool contract per invocation, implementing Rung 3 (Grounding) of the verification ladder from the ABW team's *Verification for Agent Ecologies* paper (2026-08-15). The contract declares which output fields must be sourced from successful tool calls vs. inferred by the LLM. Unsourced fields are nulled before the response persists; narrative is scanned for leaked removed values. The five-valued provenance vocabulary (Sourced / Inferred / UncommissionedInference / Narrative / Unsourced) distinguishes commissioned judgment from uncommissioned inference — do not collapse the latter into Unsourced. See [Verification for Agent Ecologies](../verification-for-agent-ecologies.md).
