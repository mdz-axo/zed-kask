...
**Run 2 — rate-path family (2026-09-20, decision-delta ladder).** Series `economics:KXFEDDECISION` (registered, live-verified), family `policy_interest_rate`. The decision-delta extractor (commit `728cdf85b7`) parsed 55 contracts: 33 point orientations (11 meetings × hike/cut/hold), 22 open tails withheld with surfaced reasons. Ladder: 3m (Dec 9 2026, 80.7d) hike 0.665 / hold 0.305 / cut 0.015 (single-cohort, mids match raw contract midpoints exactly); 6m (Mar 96.7% + Apr 3.3%, interpolated) hike 0.2658 / hold 0.6642 / cut 0.0574. 1m/2m withheld — the Oct 28 meeting (38d) falls between the fixed bucket windows; its contracts are priced but unpublished. (Window gap since fixed — contiguous midpoint bands, commits `be35743964`/`236c04d5b8`, plus the cohort-tolerance fix `49c1ade8cf`; the Oct-28 trio now forms and publishes the 1m bucket.) Economic context operator-accepted (delta space: reference 0, volatility 25bp, stable default).

Primary wedge — 25bp hike at the Oct 28 meeting: market leg (Kalshi `KXFEDDECISION-26OCT-H25`, direct API fetch) mid 0.545, spread 0.01; traditional leg (a) fed funds futures via CME FedWatch 0.574 (2026-09-18 vintage, corroborated 55.1% on 09-17 and 55% a fortnight prior). Gate: divergence 2.9pp > 1.0pp cost band → formally divergent. Decomposition: W3 exact (same written event both legs); W4 first-order — the futures reading moved 2.3pp in one day and is two days stale, and DR-AS puts the contract's own 1-hour 95% movement band at ±9.3pp (adverse selection 0.0020 dominates deadline resolution 0.00027); W2 empty (policy_interest_rate bucket sample_size 0, stale). Verdict: **monitoring signal** — the direction (prediction market less hawkish than futures for October) is recorded, the magnitude sits inside observation noise. Path context: the prediction market implies ~1.2 expected hikes by end-2026 vs the SEP median's ~1.0 (16/18 participants ≥1 more hike) — the prediction market is the more hawkish leg at the path level. Pre-resolution observations: Oct H25 0.545, Dec H25 0.665 (2026-09-20). Full report: `cmp-gap-run2-ratepath` (companies-mcp reports).
...
**Resolved 2026-09-20 (commits `be35743964`, `236c04d5b8`).** Series-scoped scans attribute a zero (`zero_scan_reason` names the per-provider dispositions — never a silent no-op); scan responses carry snapshotted market identities (`snapshot_markets` per provider, each with the market key, its calibration bucket, and the probability-at-observation); `market_lookup` records expose `clob_asset_ids` (the websocket path is pairable); and the `series` filter semantics are documented at the tool seam — Kalshi phases scoped, Polymarket leg unscoped, surfaced via `series_scope`. Bucket derivation is family-first on writer and reader sides (`BaseEconomicObject::label()` keys, e.g. `policy_interest_rate`; never per-event ticker keys), so W2 accrual and tier demotion close on the same key.

## 8. Roadmap

- **Run 2 — rate-path family (executed 2026-09-20; monitoring signal).** Policy-path prediction markets vs (a) fed funds futures-implied odds and (c) SEP/FEDTARMD medians. First family to exercise the full CMP ladder and a real (a)-leg. **Registered 2026-09-20**: `economics:KXFEDDECISION` in `prediction_markets.base_events` (user settings), live-verified — the registration gate passes and `market_cmp_indices` resolves family `policy_interest_rate` (55 contracts fetched). **Probe found two gaps.** (1) Strike extraction: the builder rejected all 55 decision-style contracts with surfaced reasons (`no extractable strike from 'Will the Federal Reserve Hike rates by 25bps…'`) — `extract_strike` parses level-space titles only ($X, X%, above/below/at); decision-delta contracts (H0/H25/H26/C25/C26) need a bp-change branch before the rate-path CMP ladder can run — landed (commit `728cdf85b7`); **run 2 executed 2026-09-20** (see §6 and report `cmp-gap-run2-ratepath`). New followups filed: the store-side curve is orientation-blind for decision families; `market_ladder` `time_to_maturity` is computed from market `updated_time` instead of now; the 1m/2m bucket-window gap leaves the nearest meeting (~38d) unpublished. (2) Context staleness: the curated default context for the rates family is Q3-2024 vintage (reference 5.375) — a live operator-accepted context is mandatory (DFF 3.88 as of 2026-09-17 after a +25bp move; FEDTARMD/SEP path 4.1 → 3.9 → 3.6). Filed followup: `market_cmp_context_suggest` classifies through the text classifier and cannot read a bare series ticker (family 'unknown', generic default for KXFEDDECISION) while `market_cmp_indices` uses the catalog classifier — the proposal side should classify through the same catalog path. **Resolved 2026-09-20 (commits `be35743964`, `236c04d5b8`, `49c1ade8cf`):** the store side is orientation-aware — `market_cmp_index_store` persists one constant-maturity curve portfolio per orientation (`cmp:{series}:{orientation}`), and `market_cmp_portfolio_store` keys solved portfolios by `(bucket, orientation)`; `market_ladder` measures `time_to_maturity` from observation time, never stale quote timestamps; maturity windows are contiguous midpoint bands (exact-boundary contracts dual-eligible) and the cohort solver's tolerance is the window's extent from the target, so the Oct-28 meeting (~38d) forms and publishes the 1m bucket — the window fix alone made the bucket available while the solver's symmetric ±7.5d tolerance still withheld every 1m index, caught in live verification and fixed in `49c1ade8cf`; and `market_cmp_context_suggest` classifies catalog-first (text-signature fallback, `classification_source` field) and proposes live economic context.
**Sixth defect surfaced by live verification (2026-09-20, fixed in commit `3493e83b6c`):** solved-portfolio constituents carried crossed market symbols — `construct_cmp_index_set` stripped the adapter-level `market_index` when narrowing to per-orientation slices, so the solver's local positions were persisted as `market_ids` lookups (the 1m:increase Oct-28 H25 constituent, price 0.545/38.5d, was labeled `KXFEDDECISION-28JAN-H26`). Probabilities and maturities were correct; only the symbol provenance crossed. Adapter indices now survive the narrowing (remapped after solving, pinned through cohort and bracket paths by test).
...
- **Calibration.** Periodic `market_check_resolutions` scans; the §7 followups are resolved (resolution record in §7) — per-market registration is now verifiable from scan-response identities.
