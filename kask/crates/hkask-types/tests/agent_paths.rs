...
use hkask_types::agent_paths::{
    MCP_DIR, SKILLS_DIR, mcp_server_db, resolve_data_dir,
...
fn storage_layout_all_classes_resolve_under_one_root() {
    ...
    // The surviving class dirs are distinct, non-empty, human-readable strings.
    // (AGENTS_DIR and THREADS_DIR were removed with their dead helpers; the
    // classes they named are still pinned by path-literal assertions below.)
    let class_dirs = [MCP_DIR, SKILLS_DIR];
    ...
    // Each class's path resolves under the same root.
    ...
    let skills_path = resolve_under_data_dir(std::path::Path::new("skills"));
    let threads_path = resolve_under_data_dir(std::path::Path::new("threads/threads.db"));
