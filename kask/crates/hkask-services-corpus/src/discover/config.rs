...
use crate::embed::{
    ChunkingConfig, CorpusConfig, DimensionCentroid, EntityConfig, FoundationalRule, TagSet,
...
// ── Literary persona generation ───────────────────────────────────────────

/// Hand-curated inputs for a literary persona corpus config.
///
/// Literary personas (woolf, hemingway, gentle-lovelace, john-brooks, etc.)
/// carry fields that are not derivable from `DiscoveredWork`: foundational
/// rules (critical essays embedded as passages), dimension centroids with
/// hand-tuned weights, orthogonal tag sets, and per-type tag weights. This
/// spec carries those inputs so `generate_persona_yaml` can produce the
/// YAML as a derived artifact of `CorpusConfig` rather than a parallel
/// hand-maintained copy.
///
/// Every field is `Option` so a caller provides only what differs from
/// `default_corpus_config`. Required fields (`author_slug`, `works`) are
/// not optional.
#[derive(Debug, Clone, Default)]
pub struct PersonaSpec {
    /// Hand-curated foundational rules (critical essays, style guides).
    pub foundational_rules: Vec<FoundationalRule>,
    /// Per-dimension centroid configuration (gentle/schriver/hopper/lovelace).
    ...
    pub triple_classifier: Option<String>,
    /// Corpus type discriminator: "literary" or "academic".
    pub corpus_type: Option<String>,
    /// Custom chunking (overrides the default 50/200/".!? ").
    pub chunking: Option<ChunkingConfig>,
    /// Custom validation (overrides the default 0.25/3/7).
    pub validation: Option<ValidationConfig>,
    /// Custom budget (overrides the default PerPage{3750}).
    pub budget: Option<BudgetConfig>,
    /// Custom embedding batch size (overrides the default 64).
    pub batch_size: Option<usize>,
    /// Entity declarations for tagging.
    pub entities: Option<EntityConfig>,
    /// Declared methods with signal thresholds.
    pub methods: Vec<DeclaredMethod>,
}

/// Generate a persona corpus.yaml from hand-curated literary inputs.
///
/// Starts from `default_corpus_config(author_slug)` and overlays the spec's
/// non-empty fields, then serializes to YAML. This is the literary analogue
/// of `generate_corpus_yaml` (which serves the academic discovery path):
/// both produce a `CorpusConfig`-shaped YAML, but this one takes hand-curated
/// literary fields instead of `DiscoveredWork`s.
///
/// Use this to (re)generate any persona corpus.yaml so it stays a derived
/// artifact of `CorpusConfig` and cannot drift. The output is guaranteed to
/// parse via `EmbedService::parse_config` because it is serialized from a
/// valid `CorpusConfig`.
///
/// \[P5\] Motivating: Essentialism — eliminates the parallel hand-maintenance
/// path that caused BUG-001 (stale YAML vs evolved struct).
///
/// pre:  author_slug must be non-empty; works must be non-empty; output_dir must exist
/// post: corpus.yaml is written to output_dir; returns PathBuf to the written file;
///       Err on serialization or I/O failure
#[must_use = "result must be used"]
pub fn generate_persona_yaml(
    author_slug: &str,
    works: &[Work],
    spec: &PersonaSpec,
    output_dir: &Path,
    ...
    tracing::info!(
        ...
        operation = "generate_persona_yaml",
        author = %author_slug,
        work_count = works.len(),
        corpus_type = ?spec.corpus_type.as_deref().unwrap_or("literary"),
        "REG"
    );

    let mut config = default_corpus_config(author_slug);
    config.works = works.to_vec();
    config.foundational_rules = spec.foundational_rules.clone();
    config.dimension_centroids = spec.dimension_centroids.clone();
    config.tag_sets = spec.tag_sets.clone();
    config.tag_weights = spec.tag_weights.clone();
    config.methods = spec.methods.clone();

    if let Some(ref corpus_type) = spec.corpus_type {
        config.corpus_type = corpus_type.clone();
    }
    if let Some(ref classifier) = spec.classifier {
        config.classifier = classifier.clone();
    }
    if let Some(ref triple_classifier) = spec.triple_classifier {
        config.triple_classifier = triple_classifier.clone();
    }
    if let Some(ref chunking) = spec.chunking {
        config.chunking = chunking.clone();
    }
    if let Some(ref validation) = spec.validation {
        config.validation = validation.clone();
    }
    ...
    if let Some(ref entities) = spec.entities {
        config.entities = entities.clone();
    }

    let config_yaml = serde_yaml_neo::to_string(&config).map_err(|e| {
        let msg = format!("Failed to serialize persona corpus config: {e}");
        ServiceError::Domain {
            domain: DomainKind::Wallet,
            kind: ErrorKind::ServiceUnavailable,
            source: Some(Box::new(e)),
    ...
    std::fs::write(&config_path, &config_yaml).map_err(|e| {
        let msg = format!(
            "Failed to write persona corpus.yaml to '{}': {e}",
            config_path.display()
        );
        ServiceError::Domain {
            domain: DomainKind::Wallet,
