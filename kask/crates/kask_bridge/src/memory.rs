...
// ── LazyMemoryPort (defers RealMemoryPort until userpod is known) ───────────

/// A `ThreadMemoryPort` that starts as a no-op and upgrades to a real port
/// once the userpod identity is known.
///
/// The `agent` crate's global hook (`set_memory_port`) uses `OnceLock`, so it
/// can only be called once. This wrapper is installed at startup (before the
/// Zed user is necessarily logged in) and `upgrade()` is called later when
/// `UserStore::current_user()` resolves the username.
///
/// Until `upgrade()` is called, `ingest_turn` delegates to `LoggingMemoryPort`
/// (logs and discards). After `upgrade()`, it delegates to the `BridgeMemoryPort`
/// wrapping a `RealMemoryPort`.
///
/// `upgrade()` is idempotent — calling it more than once is a no-op after the
/// first successful upgrade.
pub struct LazyMemoryPort {
    inner: Mutex<LazyMemoryState>,
}

enum LazyMemoryState {
    /// Initial state — logs and discards.
    Logging(LoggingMemoryPort),
    /// Upgraded — delegates to a real memory port.
    Real(BridgeMemoryPort),
    /// Upgrade failed — logs and discards, with the error recorded for diagnostics.
    Failed(String),
}

impl LazyMemoryPort {
    /// Construct a lazy port in the logging (no-op) state.
    pub fn new() -> Self {
        Self {
            inner: Mutex::new(LazyMemoryState::Logging(LoggingMemoryPort::new())),
        }
    }

    /// Upgrade to a real memory port.
    ///
    /// Call this once the userpod identity is known (from `UserStore::current_user()`).
    /// If construction of the `RealMemoryPort` fails, the port stays in a
    /// logging state with the error recorded.
    ///
    /// Idempotent: subsequent calls after a successful upgrade are no-ops.
    pub fn upgrade(&self, real: BridgeMemoryPort) {
        let mut state = self.inner.lock().expect("LazyMemoryPort mutex poisoned");
        match &*state {
            LazyMemoryState::Logging(_) | LazyMemoryState::Failed(_) => {
                *state = LazyMemoryState::Real(real);
            }
            LazyMemoryState::Real(_) => {
                // Already upgraded — ignore.
            }
        }
    }

    /// Record a failed upgrade so diagnostics can surface it.
    pub fn fail_upgrade(&self, error: String) {
        let mut state = self.inner.lock().expect("LazyMemoryPort mutex poisoned");
        match &*state {
            LazyMemoryState::Logging(_) => {
                *state = LazyMemoryState::Failed(error);
            }
            _ => {
                // Already upgraded or already failed — don't overwrite.
            }
        }
    }
}

impl Default for LazyMemoryPort {
    fn default() -> Self {
        Self::new()
    }
}

impl agent::ThreadMemoryPort for LazyMemoryPort {
    fn ingest_turn(
        &self,
        record: agent::ThreadTurnRecord,
    ) -> Pin<Box<dyn Future<Output = Result<(), String>> + Send + '_>> {
        let state = self.inner.lock().expect("LazyMemoryPort mutex poisoned");
        match &*state {
            LazyMemoryState::Logging(port) => port.ingest_turn(record),
            LazyMemoryState::Real(port) => port.ingest_turn(record),
            LazyMemoryState::Failed(err) => {
                let err = err.clone();
                Box::pin(async move {
                    log::warn!("Memory ingestion skipped — upgrade failed: {err}");
                    Ok(())
                })
            }
        }
