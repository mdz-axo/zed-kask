...
pub struct RealMemoryPort {
    ...
    /// silently.
    //
    // The user/zed agent has NO memory store. The user is human and has their
    // own memory. Only the curator, replicas, and swarm agents have kask
    // memory. When the user is chatting with the zed agent, no memory is
    // generated by the kask system — context injection continues, but memory
    // ingestion and recall are no-ops for user threads.
...
impl RealMemoryPort {
    ...
    /// router. The user/zed agent has no memory store — only the curator
    /// has memory.
    ///
...
impl MemoryPort for RealMemoryPort {
    ...
    fn recall_context<'a>(
        ...
        // User/zed agent has no memory. Only the curator has memory.
