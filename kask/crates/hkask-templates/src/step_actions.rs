...
impl StepMachine {
    ...
    pub(crate) async fn execute_tool_invoke(
        ...
        let tool_result =
            match tokio::time::timeout(timeout_dur, invoke_tool(tools, mcp_ref, input)).await {
                Ok(inner) => inner,
                Err(_elapsed) => {
                    // Record the timed-out tool call before propagating.                    // out" from "tool never called" — a timed-out call that
        ...
        // `{"tool": "server/tool_name", "ok": true/false, "result": <value>}`.        // can value-match (Truth rung) — without it, "sourced" only means
    ...
    pub(crate) async fn execute_tool_batch(
        ...
        let results = match tokio::time::timeout(timeout_dur, join_all(tool_futs)).await {
            ...
            Err(_elapsed) => {
                ...
                // recording loop below is unreachable — without this, a                // cannot distinguish "all tools timed out" from "no tools
        ...
        let elapsed_ms = batch_started.elapsed().as_millis();        // entry is recorded with its rendered MCP ref and success/failure
        ...
        // resolved reference that `execute_tool_invoke` would record —        // so an unrendered template string would never match.
    ...
    pub(crate) async fn execute_flowdef(
        ...
        let sub_convergence =
            crate::convergence::ConvergenceTracker::new(sub_manifest.convergence.max_iterations, sub_manifest.convergence.min_iterations, sub_manifest.convergence.threshold);
