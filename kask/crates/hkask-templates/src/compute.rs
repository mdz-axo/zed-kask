...
mod tests {
    ...
    fn parse_round_trips() {
        assert_eq!(
            ComputeRef::parse("lisp.eval").unwrap(),
            ComputeRef::LispEval
        );
        assert_eq!(
            ComputeRef::parse("shell.exec").unwrap(),
            ComputeRef::ShellExec
        );
    }
    ...
    fn parse_rejects_unknown() {
        assert!(ComputeRef::parse("bayesian_update").is_err());
    }

    #[test]
    fn dispatch_unknown_errors() {
        assert!(dispatch_compute("brier_score", &serde_json::json!({})).is_err());
    }

    #[test]
    fn lisp_eval_basic() {
        let r = dispatch_compute("lisp.eval", &serde_json::json!({"form": "(+ 1 2)"})).unwrap();
        assert_eq!(r, 3);
    }

    #[test]
    fn lisp_eval_with_env() {
        let r = dispatch_compute(
            "lisp.eval",
            &serde_json::json!({"form": "(+ x 1)", "env": {"x": 41}}),
        )
        .unwrap();
        assert_eq!(r, 42);
    }
    ...
    #[test]
    fn lisp_eval_missing_form_errors() {
        assert!(dispatch_compute("lisp.eval", &serde_json::json!({})).is_err());
    }
}
