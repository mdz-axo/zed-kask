...
//! `kask_bridge::concurrency`). Consumers acquire a permit before issuing a
//! cloud inference or tool call, then call `on_success` or `on_throttle` after
//! the call completes. The permit is released when the returned guard drops.
//!
