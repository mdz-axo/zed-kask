...
mod tests {
    ...
    #[test]
    fn all_concepts_are_5w1h_namespaced() {
        for concept in ALL_CONCEPTS {
            assert!(
                concept.starts_with("5w1h:"),
                "5W1H concept must be 5w1h-namespaced: {concept}"
            );
        }
    }

    #[test]
    fn six_dimensions_present() {
        assert_eq!(
            ALL_CONCEPTS.len(),
            6,
            "exactly six interrogative dimensions"
        );
        assert_eq!(DIMENSION_LABELS.len(), 6);
    }

    #[test]
    fn label_concept_round_trip() {
        for label in DIMENSION_LABELS {
            let concept = concept_for_label(label).expect("label must resolve");
            let back = label_for_concept(concept).expect("concept must resolve");
            assert_eq!(*label, back);
        }
    }

    #[test]
    fn state_axis_dimensions_are_dc() {
        for concept in [WHO, WHAT, WHEN, WHERE] {
            assert_eq!(
                axis_for_concept(concept),
                Some(crate::axis::OntologyAxis::DcBibo),
                "{concept} must be state-axis (DC)"
            );
        }
    }

    #[test]
    fn process_axis_dimensions_are_pko() {
        for concept in [WHY, HOW] {
            assert_eq!(
                axis_for_concept(concept),
                Some(crate::axis::OntologyAxis::Pko),
                "{concept} must be process-axis (PKO)"
            );
        }
    }
    ...
    fn unknown_label_returns_none() {
        assert!(concept_for_label("unknown").is_none());
        assert!(label_for_concept("not_a_concept").is_none());
    }
}
