...
mod tests {
    ...
    #[test]
    fn sumo_seed_concepts_are_sumo_namespaced() {
        for concept in ALL_CONCEPTS {
            assert!(
                concept.starts_with("sumo:"),
                "SUMO concept must be sumo-namespaced: {concept}"
            );
        }
    }
    ...
    #[test]
    fn entity_is_root() {
        assert_eq!(ENTITY, "sumo:Entity");
    }

    #[test]
    fn process_is_dynamic_counterpart_to_object() {
        // SUMO's foundational split: Object (static) vs Process (dynamic).
        assert_ne!(OBJECT, PROCESS);
        assert!(OBJECT.ends_with("Object"));
        assert!(PROCESS.ends_with("Process"));
    }

    #[test]
    fn kg_relations_present() {
        // The four relations the KG triple extractor uses for cross-document
        // linkage: subclass (is-a), part (part-of), attribute (has-property),
        // causes (causal dependency).
        for relation in [SUBCLASS, PART, ATTRIBUTE, CAUSES] {
            assert!(
                ALL_CONCEPTS.contains(&relation),
                "KG relation {relation} must be in ALL_CONCEPTS"
            );
        }
    }
}
