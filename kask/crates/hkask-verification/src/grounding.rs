...
/// Generate a minimal grounding contract for an administrative or
/// data-providing MCP tool (core-level grounding).
///
/// These tools — swarm management (list/clone/remove/get), kanban board
/// listing, equity research (transcript/valuation/forecast), and
/// infrastructure (web search, codegraph stats) — return their output
/// directly from the tool's execution, not LLM-synthesized analysis. The
/// core-level grounding path (`tool_span.rs`) passes `tool_calls: &[]`, so
/// any field with non-empty `sources` would be nulled as Unsourced (no tool
/// call to match against). Empty `field_sources` is the correct contract:
/// all output fields are `UncommissionedInference` (kept, not nulled) when
/// the output is a JSON object, and the delegation records `unenforceable`
/// (contract exists but output isn't an object) when the output is a
/// string or array. Both paths count as "with contract" in the coverage
/// metric — the contract acknowledges the tool exists and its output is
/// the tool's direct return.
///
/// Richer contracts (with `Sourced` fields checked against `tool_calls`)
/// belong in the tool-level grounding path (`enforce_and_stamp`), where
/// the delegating tool has visibility into sub-agent tool calls.
pub fn administrative_tool_contract(tool_name: &str) -> GroundingContract {
    GroundingContract {
...
/// The set of MCP tool names that get an `administrative_tool_contract` at
/// construction. These are tools whose core-level output is the tool's direct
/// return (no LLM synthesis), so no sourced fields are expected. Registering
/// these closes the grounding coverage gap (paper §6: coverage is a metric,
/// not a pass) without creating false violations.
///
/// To extend: add the tool name here. The contract is generated by
/// [`administrative_tool_contract`]. If a tool later needs richer grounding
/// (sourced fields checked against `tool_calls`), write a dedicated contract
/// function and register it in [`VerificationStore::new`] instead.
pub const ADMINISTRATIVE_TOOL_NAMES: &[&str] = &[
    // Swarm management — list/clone/remove/get agent cards and swarms.
    ...
    "swarm_get_swarm",
    "swarm_list_agents",
    // Kanban — board listing.
    "kanban_board_list",
    // Equity research — financial data providers returning raw data.
    "company_transcript",
    "comparable_analysis",
    ...
    "expectations_gap",
    "forecast_list",
    // Infrastructure — search and codegraph metadata.
    "web_search",
    "codegraph_stats",
];
