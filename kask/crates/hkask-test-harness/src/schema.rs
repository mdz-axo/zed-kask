//! Embedded hKask database schema for test harness.
//!
//! This is a copy of `crates/hkask-storage/src/sql/schema.sql`
//! with the `$DIM` placeholder resolved to the default 1024 dimensions.
//! Kept in the harness crate to avoid a runtime dependency on storage's
//! `include_str!` path, which resolves relative to the storage crate.

/// Full hKask schema SQL with vec0 dimension resolved to 1024.
...
CREATE TABLE IF NOT EXISTS quarantined_goals (\
id TEXT PRIMARY KEY, \
...
key_id TEXT PRIMARY KEY REFERENCES api_keys(key_id), \
wallet_id TEXT NOT NULL REFERENCES wallet_balances(wallet_id), \
amount_rj INTEGER NOT NULL, \
