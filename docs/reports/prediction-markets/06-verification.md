...
1. **Settings page:** open Zed settings → Kask → *Prediction Markets* sub-page. Confirm the three fields render (Data Directory, Cache TTL, Base-Event Registry) and a value you enter persists to `settings.json` under `kask.prediction_markets`.
...
3. **Live cascade:** ask the agent *"what does the market imply about the Fed in December 2027, and anchor a scenario on it"* — confirm it calls `market_match`/`market_lookup` then `scenario_from_markets` and shows the annotated record (tier, bias, ontology).
