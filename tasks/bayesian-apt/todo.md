...
- [ ] **C0.4** 1m/3m/6m CMP indices per (family, orientation), per-venue; publish probability + weights + maturity error + reliability floor — **builder landed 2026-08-07** in `cmp_index_builder.rs`: `build_cmp_indices` reads per-family JSONL catalogs, adapts records to `EligibilityInput`, calls `construct_cmp_index_set`, wraps each `CmpIndex` with `ProvenancedCmpIndex` (family + venue). 16 tests pass (hand-checkable bracket, withhold, roll hand-off, parse-error propagation, real-catalog smoke test, human-review sample, all-families probe). All-families probe: only 3 family/venue/tenor combinations publish via bracket solver (Rates 6m Kalshi, CPI 3m Kalshi, Oil 1m Kalshi) — most families have eligible contracts but no bracket pair (contracts cluster at single maturity within each window). See `all-families-probe.md`.
- [ ] **CP-CMP** rates family produces continuous 1m/3m/6m series on both venues; ≥90% of days within tolerance; eligibility classifications human-reviewed — **post-ONT-6 + all-families probe**: classifier unblocked, but structural maturity-ladder gap limits bracket-based publication. Kalshi rates: 6m publishes (bracket); 1m/2m/3m have contracts at single maturity (no bracket). Polymarket rates: 29 eligible, 0 publish (all at year-end). CP-CMP criterion needs revision: accept single-cohort publication (`BucketedSparse`) where brackets don't exist. See `all-families-probe.md`.
- [ ] **C0.5** (new) Single-cohort fallback — integrate `BucketedSparse` method from `cmp.rs` into `cmp_portfolio.rs` as a fallback when `solve_portfolio` returns `None` but eligible contracts exist in the window. Publishes a degraded index with the maturity error surfaced (distance from cohort to target). Unblocks 5/6 families on Kalshi. See `all-families-probe.md` recommendation.
