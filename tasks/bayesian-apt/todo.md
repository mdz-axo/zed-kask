...
- [ ] **CP-CMP** rates family produces continuous 1m/3m/6m series on both venues; ≥90% of days within tolerance; eligibility classifications human-reviewed — **post-ONT-6**: classifier is no longer the bottleneck. Kalshi rates: 388 eligible, 6m Increase + 6m Stable publish (1m/2m/3m have contracts at single maturity — no bracket). Polymarket rates: 29 eligible, 0 publish (all contracts at 124d/146d — no bracket). Remaining gaps are **genuine venue characteristics** (maturity-ladder gaps), not classifier errors. Next: decide whether to relax the CP-CMP criterion for the rates family (accept 6m-only) or build indices for other families first.
