...
- [ ] **C0.4** 1m/3m/6m CMP indices per (family, orientation), per-venue; publish probability + weights + maturity error + reliability floor — **builder landed 2026-08-07** in `cmp_index_builder.rs`: `build_cmp_indices` reads per-family JSONL catalogs, adapts records to `EligibilityInput`, calls `construct_cmp_index_set`, wraps each `CmpIndex` with `ProvenancedCmpIndex` (family + venue). 15 tests pass (hand-checkable bracket, withhold, roll hand-off, parse-error propagation, real-catalog smoke test, human-review sample). Real-catalog probe: Kalshi rates publishes 3m/6m (1m/2m withheld — FOMC cadence gap); Polymarket rates publishes 0 (classifier signature gap — ONT-6 triggered). See `c0.4-decisions.md`.
- [ ] **CP-CMP** rates family produces continuous 1m/3m/6m series on both venues; ≥90% of days within tolerance; eligibility classifications human-reviewed — **partial**: Kalshi 3m/6m pass, 1m withholds (genuine FOMC cadence gap); Polymarket fails (classifier signature gap — ONT-6 triggered). Human review of 20 Kalshi rates contracts: all classify correctly as InterestRates, strikes extracted correctly. ONT-6 is the next step before CP-CMP can pass on Polymarket.
...
- [ ] **ONT-6** Rewire `economic_object.rs` and `base_event.rs` onto the FIBO-anchored classification through the corpus pipeline (delete the substring synonym-closure loop). **Triggered 2026-08-07**: C0.4 CP-CMP probe found the `InterestRates` signature doesn't cover Polymarket phrasings ("Fed's upper bound", "reach X%") — the substring classifier is the bottleneck on Polymarket rates. This is the next step.
