...
- [x] **C — "I disagree" in-place correction gesture (all 4 widgets)** — DONE. The compose-back seam is built (D21: `hkask-conversation-injector` leaf trait + `ThreadConversationInjector` prefill in `agent_ui` + `publish_injector` on activation + DIVERGENCE.md + tests). All four widgets now have the consumer: `compose_disagree_body` (provenance/subject-scoped revision request) + `on_disagree_click` → `shared_injector().inject(body, window, cx)` + `disagree_draft` visible fallback when no injector + `reg.widget.disagree` span + an "I disagree" chip in each widget's header. Tests: portfolio 3 + scenarios 3 + kanban 3 + graph 4 disagree tests, all pass (28/26/34/24 widget tests total). A refactor-architecture review confirmed D21's PREFILL approach is sound — auto-send was rejected as flaky (bypasses `send_content`). Fixed the portfolio test compile bug (VisualTestContext::update 2-arg closure) the parallel build left.
- [x] **F - inline drill-down** DONE. Portfolio widget: per-row Explain chip on attribution rows dispatches research_search with the row symbol on hkask-mcp-companies via shared_tool_invoker; result displayed inline (truncated ~500 chars). State: explain_symbol/explain_result/explain_error. 4 tests. 32 portfolio tests pass; clippy clean; downstream builds.
- [x] **D - ghost edits** DONE. Kanban widget: Evaluate button in the PendingMove consent banner alongside Confirm/Cancel. Composes an evaluation request via D21 compose-back. Extracted compose_back helper shared by C disagree + D evaluate. 3 tests. 37 kanban tests pass; clippy clean; downstream builds.
- [x] **C on the other 3 widgets** (scenarios/kanban/graph) - DONE (wired this turn).
- [ ] **I - ontology-bounded affordances** - NOT DEFERRED. The ontology audit found all 8 domains are real, in active use across 6 MCP servers + the condenser, NONE speculative. The gap is none are wired to widget affordances. Priority order: (1) OMC re-build (media server bridge was built+removed as dead code; needs re-wiring), (2) FIBO wiring (portfolio Explain should consult FIBO tag for tool selection), (3) PKO wiring (scenarios rungs + kanban moves should consult PKO), (4) Dublin Core/GOLEM/ESO/CogAT/ML-Schema (need widget consumers).