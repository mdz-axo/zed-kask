...
- **Track 3 (branch/compare — parallel, measurement-gated):** expose `VIZ_CACHE` as a version surface, gated on a measured re-ask rate.
...
9. **Scope exclusion — media playback perf:** the `code-review` should-fix findings SF-1 (foreground audio decode `media_widget.rs:171/245`), SF-2 (perpetual 30fps timer `:283-300`), SF-3 (loop fires on load-failure `:139/154`) are real but orthogonal to sovereignty. They are listed as open questions / parallel work, not tasks in this plan. SF-4 (date `unwrap_or_default` → 1970-01-01, `hkask_mcp_companies/tools/portfolio.rs:331-361`) is incidentally fixed by Track 2's portfolio fan-out, which validates args before dispatch.
...
**T7 — Measurement gate: `reg.widget.reask` span** (scope: S)
- Instrument a `reg.widget.reask` span keyed by `provenance.span_id`, emitted when a user sends a chat message within N turns/seconds of a widget render whose `provenance.tool` matches the message's intent (heuristic; precise matching is an open question). Aggregate the re-ask rate per widget-bearing turn.
- **Gate:** >15% re-ask → proceed to T8a; <5% → defer Track 3 and the Phase-4 C/F fan-out (E alone may suffice); 5–15% → human decides.
- **Acceptance criteria:** the span is emitted and queryable; the aggregate rate is computable; the gate decision is documented.
- **Verification:** run a session, confirm spans appear in the regulation store.
- **Dependencies:** T4 (needs provenance to key on). Parallel to T5/T6.

**T8a — Cache typed-handle + version key** (scope: M) — *only if T7 gate passes*
- Change `CachedWidget` (`hkask_viz_core.rs:176-194`) to retain a typed dispatch handle (or a `Box<dyn Fn() -> AnyElement>` render closure **plus** a version/branch discriminator), and change `cache_key` (`:275-279`) from a bare `u64` body hash to a key that includes a version/branch id and verifies body equality on hit (fixes the collision risk N-3).
...
- A "Keep this version / try a different assumption" affordance on a rendered widget that creates a branch (new cache entry) and a "Revert" that restores the agent's version.
- **Acceptance criteria:** branching creates a distinct cached entity; revert restores the prior body's widget.
- **Verification:** `cargo test` on the host widget; manual.
- **Dependencies:** T8a.

**T8c — Compare side-by-side UI** (scope: S) — *only if T8a*
- Render two cached widgets (agent version + user branch) side by side for comparison.
...
5. **Media playback perf (SF-1/2/3):** foreground audio decode + perpetual 30fps timer. Orthogonal to this plan; schedule as separate work. SF-4 is fixed incidentally by T5.
6. **Should Track 1/2 wire the graph widget's `set_evidence` to dispatch `scenario_quantify`?** Today it re-propagates locally only. This would close the Good-Regulator loop for the one widget that already has an effector. Not scheduled (graph widget's value is instant local feedback); open as a candidate after T4.
