...
7. **C/D/F-investigate need a compose-back seam that touches upstream `agent_ui` (a D-seam decision).** The widget (a leaf) has no kask-only path to the active conversation thread — `ConversationView::render_markdown` calls `render_agent_markdown` with `&self.workspace` but not the active thread (`active_thread()` is on `ConversationView`). In-place injection ("I disagree" → revision request back to the agent) needs either (a) extending the D18 `render_agent_markdown` + block-renderer callback signature to thread a `ConversationInjector` through to widgets, or (b) a kask global injector (`hkask-conversation-injector` leaf, like `hkask-tool-invoker`) that `ConversationView` sets on activation/thread-change. Both touch upstream `agent_ui` — a load-bearing D-seam (per .rules: add a DIVERGENCE.md entry + pinning test). The lower-risk is (b) (a setter call on activation, no signature change to `render_agent_markdown`). The kask leaf alone is not built speculatively (no consumer until the D-seam). **Decision needed:** approve the upstream D-seam for the compose-back seam, or defer C/D/F-investigate.
