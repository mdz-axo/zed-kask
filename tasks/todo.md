...
- [ ] **Live normal-quit observation.** Watcher ARMED 2026-09-08T23:02:16Z: `target/kask-quit-watch-20260908T225830Z/` (detached, read-only, 7200 s bound) tracking editor PID 90198 + 11 children by PID + start ticks; identities refreshed at arm time; detection paths pre-validated on scratch processes (`target/kask-quit-watch-selftest/` — TERMINATED/REPARENTED/ALL_TRACKED and the LEAK verdict all fired). Remaining: operator quits the editor normally (after in-flight parallel-session work finishes) and confirms the quit manner; then read `summary.txt`/`watcher.log` (CLEAN/LEAK/EXPIRED; re-arm per the README if expired). No quit observed yet. Existing connected/pending-child fixture evidence is not a live app pass.
