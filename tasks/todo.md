...
- [ ] **T16 — Wire `kask.memory.memory_life_days` into actual decay.** Curator-server + formula tests green in prior handoff; bridge historical RED observed (180 vs 30), post-restore full bridge suite 181/181 green. Sensor reads the applied store. Zed check and affected lint remain incomplete. Settings UI already exists: step 6 is NOT skipped.
- [ ] **T17 — Consolidation timer truth.** Historical RED observed: original cold-start timer never fires. Interval-native repair GREEN in 181/181 bridge suite; two-hour cadence test additionally caught and fixed the obsolete one-hour polling cap. Test harness fixes: Tokio `test-util`, moved-record clone, zero-cadence Tokio annotation. App/lint close-out remains incomplete.
...
- [ ] **T19 — MCP servers die with the session.** Duplicate shutdown compile failure fixed. Real-process RED exposed detached rmcp cleanup leaking a child; runtime now owns and awaits kill-and-reap. Second RED exposed queued startup after shutdown; terminal latch added. Final 17 unit + 14 integration tests GREEN. D51 quit hook and retained subscription implemented; Zed check and live app quit remain unverified.
...
- [ ] **Validation close-out** (owner: receiving coding agent): bridge/runtime tests above pass. `cargo check --offline --locked -p zed -j 8` timed out at 30m; forced-release lint was operator-interrupted; corrected dev-profile scoped lint timed out at 20m (17 compile + 207 check entries, through `workspace`/`agent_servers`). No compiler jobs remained afterward. Await operator-approved incremental validation window; preserve target/cache, one build at a time. Do not start T18 yet. Exact commands and results: plan.md 2026-09-08 evidence table.
