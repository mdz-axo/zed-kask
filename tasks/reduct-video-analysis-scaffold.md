...
| **Cost model** | Credits per delegation (`credits_authorized`) | Subscription floor ($40/editor/mo Professional) + pooled annual transcription hours + per-minute overage; silence excluded from billing | No per-use cost (compute only) | No per-use cost for ffmpeg/gallery; STT/vision billed by the configured inference providers |
